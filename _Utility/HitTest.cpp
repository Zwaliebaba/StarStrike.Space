...
void HitTest::SetBB(Time start, Time stop, float dt)
{
  assert(stop >= start);
...
void HitTest::UpdateBB(void)
{
  if (!m_staticF)
  {
    Vector startPosition = m_tStart == 0.0f ? GetPosition() : GetPosition() + m_velocity * m_tStart;//offset for delayed start
...
constexpr double epsilon = 1.0f / 128.0f;

void HitTest::Collide(HitTest *pHitTest, CollisionQueue *pQueue)
{
  assert(pQueue);
  const BoundingBox &hitTestBB = pHitTest->m_boundingBox;

  if ((m_boundingBox.axes[0].values[1] > hitTestBB.axes[0].values[0]) && (m_boundingBox.axes[0].values[0] < hitTestBB.axes[0].values[1]) && (m_boundingBox.axes[1].values[1] > hitTestBB.axes[1].values[0]) && (m_boundingBox.axes[1].values[0] < hitTestBB.axes[1].values[1]) && (m_boundingBox.axes[2].values[1] > hitTestBB.axes[2].values[0]) && (m_boundingBox.axes[2].values[0] < hitTestBB.axes[2].values[1]))
  {
    ...
    double c = static_cast<double>(dP * dP) - static_cast<double>(r * r);
    ...
    if (halfB > 0.0)//objects need to be closing at least a little to generate a collision
    {
      ...
      if (a > 0.0)//Shouldn't be a problem but ... float do have their limits of resolution
      {
        ...
        if (b2ac >= 0.0)
        {
          ...
          double s = sqrt(b2ac);
          tCollision = (c <= 0.0)
                         ? 0.0f//Bounding spheres already intersect
                         : static_cast<float>((halfB - s) / a);
          tMax = static_cast<float>((halfB + s) / a);

          if (tMax > m_tStop) tMax = m_tStop;
    ...
    else if (c < 0.0)
    {
      ...
      if (halfB != 0.0)
      {
        ...
        if (a > 0.0)//Shouldn't be a problem but ... float do have their limits of resolution
        {
          tMax = static_cast<float>((halfB + static_cast<float>(sqrt(halfB * halfB - a * c))) / a);
        }
      }
    ...
    if (fCollision && (tCollision >= m_tStart) && (tCollision <= m_tStop + static_cast<float>(epsilon)))
    {
      HitTestShape htsThis = m_shape;
      HitTestShape htsPHT = pHitTest->m_shape;
      if (((m_shape <= c_htsSphere) && (pHitTest->m_shape <= c_htsSphere)) || HullCollide(&tCollision, tMax + static_cast<float>(epsilon), this, &htsThis, pHitTest, &htsPHT, dP, dV)) { pQueue->addCollision(tCollision, this, htsThis, pHitTest, htsPHT); }
    }
  }
}
...
public:
  ...
  void SetPosition(const Vector &p) { m_position = p; }

  const ConvexVertex **GetAdjacencies(void) const { return m_adjacencies; }
...
public:
  ...
  Vector GetExtreme(const Vector &direction)
  {
    ...
    if (l < 0.01f) return Vector(0.0f, 0.0f, 0.0f);
...
public:
  ...
  void *operator new(size_t size, int nVertices, int nAdjacencies) { return ::new char [size + sizeof(ConvexVertex) * nVertices + sizeof(ConvexVertex *) * (nAdjacencies + nVertices)]; }

  const ConvexVertex *GetExtremeVertex(const Vector &direction) const { return m_pcvExtremes[((direction.x < 0.0) ? 0 : 1) + ((direction.y < 0.0) ? 0 : 2) + ((direction.z < 0.0) ? 0 : 4)]; }

  void CalculateExtremeVertices(void)
  {

    static const float octants[][3] = {{-1.0f, -1.0f, -1.0f}, {1.0f, -1.0f, -1.0f}, {-1.0f, 1.0f, -1.0f}, {1.0f, 1.0f, -1.0f}, {-1.0f, -1.0f, 1.0f}, {1.0f, -1.0f, 1.0f}, {-1.0f, 1.0f, 1.0f}, {1.0f, 1.0f, 1.0f}};
    ...
    for (int octant = 0; (octant < 8); octant++)
    {
      ...
      while (true)
      {
        const ConvexVertex *pcvNext = nullptr;
        ...
        } while (*(++ppcvAdjacent) != nullptr);

        if (pcvNext) pcv = pcvNext;
        else
        {
...
private:
  const ConvexVertex *vertex0(void) const { return ((const ConvexVertex *) (((char *) this) + sizeof(*this))); }
  const ConvexVertex **adjacency0(void) const { return ((const ConvexVertex **) (vertex0() + m_nVertices)); }
...
{
public:
  MultiHull(int nHulls, float originalRadius, const Vector &ellipseEquation, float ellipseRadiusMultiplier)
  ...
  ~MultiHull(void) { for (int i = 0; (i < m_nHulls); i++) delete GetSingleHull(i); }

  void *operator new(size_t size, int nHulls) { return ::new char [size + sizeof(SingleHull *) * nHulls]; }

  int GetNhulls(void) const { return m_nHulls; }
...
private:
  SingleHull **hull0(void) const { return ((SingleHull **) (((char *) this) + sizeof(*this))); }
...
{
public:
  BoundingHull(IObject *d, bool staticF, const MultiHull *pMultiHull)
      ...
      m_pMultiHull(pMultiHull)
  {
    ...
    for (int i = pMultiHull->GetNhulls() - 1; (i >= 0); i--) pcvRecent[i] = pMultiHull->GetSingleHull(i)->vertex0();
  }

  void *operator new(size_t size, const MultiHull *pMultiHull) { return ::new char [size + sizeof(ConvexVertex *) * pMultiHull->GetNhulls()]; }

  void UpdateStaticBB(void) override { HitTest::UpdateStaticBB(m_desiredRadius * m_pMultiHull->m_ellipseRadiusMultiplier); }

  Vector GetCenter(HitTestShape hts) const override
  {
  ...
  Vector GetEllipseExtreme(const Vector &direction)
  {
    ...
    double rk = 1.0 / sqrt(a2 * direction.x * direction.x + b2 * direction.y * direction.y + c2 * direction.z * direction.z);

    Vector extreme(static_cast<float>(direction.x * a2 * rk), static_cast<float>(direction.y * b2 * rk), static_cast<float>(direction.z * c2 * rk));
...
#pragma optimize ("", on)
  ...
  void SetShape(HitTestShape shape) override
  {
    ...
    if (s != m_shape)
    {
      ...
      if (m_shape == c_htsEllipse) m_radius = m_desiredRadius * m_pMultiHull->m_ellipseRadiusMultiplier;
      else m_radius = m_desiredRadius;
    }
  }
  ...
  void SetRadius(float r) override
  {
    ...
    if (m_shape == c_htsEllipse) m_radius = r * m_pMultiHull->m_ellipseRadiusMultiplier;
    else m_radius = r;
  }

  float GetScale(void) const override { return m_scale; }

  const Vector *GetEllipseEquation(void) const override { return &m_ellipseEquation; }

  const Vector GetFrameOffset(const char *pszFrameName) const override { return m_pMultiHull->GetFrameOffset(pszFrameName) * m_scale; }

  const Vector &GetFrameForward(const char *pszFrameName) const override { return m_pMultiHull->GetFrameOffset(pszFrameName); }
  const FrameDataUTL *GetFrame(const char *pszFrameName) const override { return m_pMultiHull->GetFrame(pszFrameName); }

private:
  const ConvexVertex **recentVertex0(void) const { return ((const ConvexVertex **) (((char *) this) + sizeof(*this))); }
...
{
public:
  ~CachedMultiHull(void) { delete m_pMultiHull; }

  char m_name[c_cbName];
  MultiHull *m_pMultiHull;

  friend bool operator!=(const CachedMultiHull &r1, const CachedMultiHull &r2) { return false; }

};

using CachedList = Slist_utl<CachedMultiHull>;
using CachedLink = Slink_utl<CachedMultiHull>;

CachedList cachedMultiHulls;

MultiHullBase *HitTest::Load(const char *pszFileName)
{
  if ((!pszFileName) || (pszFileName[0] == '\0')) return nullptr;

  MultiHull *pMultiHull = nullptr;
  ...
  bool bFound = false;
  {
    for (CachedLink *pcl = cachedMultiHulls.first(); (pcl != nullptr); pcl = pcl->next())
    {
  ...
  if (!bFound)
  {
    ...
    HRESULT hr = UTL::getFile(pszFileName, ".cvh", artwork, true, true);
    ...
    if (hr == S_OK)//Don't bother to try and read a file that doesn't contain a valid convex hull
    {
      ...
      if (fileIn)
      {
        constexpr int c_maxLine = 512;
        char line[c_maxLine];

        float radius;
        Vector ee;//ellipse equation
        float erm;
        if (fgets(line, c_maxLine, fileIn) && (sscanf(line, "%f %f %f %f %f", &radius, &ee.x, &ee.y, &ee.z, &erm) == 5))
        {
          int nHulls;
          if (fgets(line, c_maxLine, fileIn) && (sscanf(line, "%d", &nHulls) == 1))
          {
            assert(nHulls > 0);
            ...
            for (int hullID = nHulls - 1; (hullID >= 0); hullID--)
            {
              ...
              int nAdjacencies;
              Vector center;
              if (fgets(line, c_maxLine, fileIn) && (sscanf(line, "%d %d %f %f %f", &nVertices, &nAdjacencies, &(center.x), &(center.y), &(center.z)) == 5))
              {
                assert(nVertices > 0);
                ...
                auto psh = new(nVertices, nAdjacencies) SingleHull(nVertices, center);
                *(ppsh++) = psh;

                //Hack alert: force a cast away from constness so that we can modify the vertex
                //as we read it in.
                auto pcv = (ConvexVertex *) (psh->vertex0());
                ...
                do
                {
                  Vector xyz;
                  int n;
                  if (fgets(line, c_maxLine, fileIn) && (sscanf(line, "%f %f %f %d", &xyz.x, &xyz.y, &xyz.z, &n) == 4))
                  {
                    assert(n > 0);
        ...
        while (fgets(line, c_maxLine, fileIn))
        {
          ...
          sscanf(line, "%f %f %f", &(pfd->position.x), &(pfd->position.y), &(pfd->position.z));

          ZVerify(fgets(line, c_maxLine, fileIn));
          sscanf(line, "%f %f %f", &(pfd->forward.x), &(pfd->forward.y), &(pfd->forward.z));
...
HitTest *HitTest::Create(const char *pszFileName, IObject *data, bool staticF, HitTestShape htsDefault)
{
  HitTest *pHitTest = nullptr;

#ifndef DREAMCAST
  if (pszFileName)
  {
    //An unsafe upcast ... but we know better.
    auto pMultiHull = static_cast<MultiHull *>(Load(pszFileName));
    if (pMultiHull) { pHitTest = new(pMultiHull) BoundingHull(data, staticF, pMultiHull); }
  }
#endif //dreamcast
  ...
  if (!pHitTest)
  {
    ...
    {
      case c_htsSphere:
        pHitTest = static_cast<HitTest *>(new BoundingSphere(data, staticF));
        break;
      case c_htsPoint:
        pHitTest = static_cast<HitTest *>(new BoundingPoint(data, staticF));
        break;
      case c_htsCone:
        pHitTest = static_cast<HitTest *>(new BoundingCone(data, staticF));
        break;
...
Vector HitTest::GetMinExtreme(HitTestShape hts, const Vector &position, const Orientation &orientation, const Vector &direction)
{
  switch (hts)
  {
    case c_htsPoint:
    ...
    case c_htsEllipse:
      return position + static_cast<BoundingHull *>(this)->GetEllipseExtreme(orientation.TimesInverse(-direction)) * orientation;

    case c_htsCone:
      return position + static_cast<BoundingCone *>(this)->GetExtreme(orientation.TimesInverse(-direction)) * orientation;

    case c_htsSphere:
      return position - direction * m_radius;

    default:
      return position + static_cast<BoundingHull *>(this)->GetHullExtreme(hts, orientation.TimesInverse(-direction)) * orientation;
  }
}

Vector HitTest::GetMaxExtreme(HitTestShape hts, const Vector &direction)
{
  switch (hts)
  {
    case c_htsPoint:
    ...
    case c_htsEllipse:
      return static_cast<BoundingHull *>(this)->GetEllipseExtreme(direction);

    case c_htsCone:
      return static_cast<BoundingCone *>(this)->GetExtreme(direction);

    case c_htsSphere:
      return direction * m_radius;

    default:
      return static_cast<BoundingHull *>(this)->GetHullExtreme(hts, direction);
  }
}

static bool DoGilbert(HitTest *phtObjectA, HitTestShape htsA, const Vector &positionStart, const Vector &positionStop, const Vector &dV, const Orientation &orientationHullA, HitTest *phtObjectB, HitTestShape htsB, Vector simplex[4]);

bool HitTest::HullCollide(float *tStart, float tMax, HitTest *phtHullA, HitTestShape *phtsA, HitTest *phtHullB, HitTestShape *phtsB, const Vector &dP, const Vector &dV)
{
  assert(tStart);
  assert(phtHullA);
  assert(phtHullB);
  ...
  if ((phtHullA->m_shape == c_htsPoint) && (phtHullB->m_shape == c_htsEllipse))
  {
    ...
    if (b > 0.0)
    {
      ...
      if (bac >= 0.0)
      {
        ...
        float t = *tStart + static_cast<float>((b - sqrt(bac)) / a);
        if (t <= tMax)
        {
          *tStart = t;
          *phtsA = phtHullA->m_shape;
  ...
  constexpr double c_maxDelta = 0.25;
  ...
  HitTestShape htsB = ((phtHullB->m_pvUseTrueShapeSelf == nullptr) || (phtHullB->m_pvUseTrueShapeSelf != phtHullA->m_pvUseTrueShapeOther)) ? phtHullB->m_shape : phtHullB->m_shapeTrue;
  ...
  do
  {
    ...
    do
    {
      if (IntervalCollide(tOriginalStart, tMax, (speed == 0.0) ? FLT_MAX : static_cast<float>(c_maxDelta / speed), phtHullA, htsAcurrent, phtHullB, htsBcurrent, p, v, o, tStart))
      {
        bCollision = true;
        *phtsA = htsAcurrent;
        *phtsB = htsBcurrent;
...
bool HitTest::IntervalCollide(float tStart, float tStop, float maxDeltaT, HitTest *phtHullA, HitTestShape htsA, HitTest *phtHullB, HitTestShape htsB, const Vector &dP, const Vector &dV, const Orientation &orientationA, float *tCollision)
{
  ...
  if (htsA >= c_htsConvexHullMin) direction += phtHullA->GetCenter(htsA) * orientationA;
  if (htsB >= c_htsConvexHullMin) direction -= phtHullB->GetCenter(htsB);

  if ((direction.x == 0.0f) && (direction.y == 0.0f) && (direction.z == 0.0f)) return true;
  ...
  while (true)
  {
    ...
    extremesHullA[attempt] = phtHullA->GetMinExtreme(htsA, (direction * dV) >= 0.0f ? positionStop : positionStart, orientationA, direction);
    ...
    if (attempt++ >= c_maxAttempts - 1) break;
  ...
  {
    int i = 0;
    do { simplex[i] = extremesHullA[i] - extremesHullB[i]; } while (i++ < 3);
  }

  if (!DoGilbert(phtHullA, htsA, positionStart, positionStop, dV, orientationA, phtHullB, htsB, simplex))
  {
  ...
  //split the interval in two and try again
  return IntervalCollide(tStart, tMiddle, maxDeltaT, phtHullA, htsA, phtHullB, htsB, dP, dV, orientationA, tCollision) || IntervalCollide(tMiddle, tStop, maxDeltaT, phtHullA, htsA, phtHullB, htsB, dP, dV, orientationA, tCollision);
}

static int closest_vertex(int v0, const Vector p[],
                          //double      lambda[],
                          int index[], Vector *cp)
{
  //lambda[0] = 1;
...
static int closest_edge(double e0, double e1, int ie0, int ie1, const Vector p[], int index[], Vector *cp)
{
  double esum_inv = 1.0f / (e0 + e1);

  float lambda[2];
  lambda[0] = static_cast<float>(e0 * esum_inv);
  lambda[1] = static_cast<float>(e1 * esum_inv);
...
static int closest_face(double f0, double f1, double f2, int if0, int if1, int if2, const Vector p[],
                        //double        lambda[],
                        int index[], Vector *cp)
{
  ...
  float lambda[3];
  lambda[0] = static_cast<float>(f0 * fsum_inv);
  lambda[1] = static_cast<float>(f1 * fsum_inv);
  lambda[2] = static_cast<float>(f2 * fsum_inv);

  index[0] = if0;
  index[1] = if1;
  index[2] = if2;

  *cp = lambda[0] * p[if0] + lambda[1] * p[if1] + lambda[2] * p[if2];
...
*/
static int Johnson2(const Vector p[2], Vector *cp, int index[])
{
  Vector d = p[1] - p[0];
  ...
  *cp = p[0] + static_cast<float>(l0 / (l0 - l1)) * d;
...
static int Johnson3(const Vector p[3], Vector *cp, int index[])
{
  ...
  //  subset3             ({P0, P1, P2})
  static constexpr int subset1[3] = {0, 1, 2};
  static constexpr int subset2[3][3] = {{-1, 3, 4}, {3, -1, 5}, {4, 5, -1}};
  static constexpr int subset3 = 6;

  double dp[3][3];
  {
    for (int i = 0; (i < 3); i++)
      for (int j = i; (j < 3); j++)
        dp[i][j] = dp[j][i] = static_cast<double>(p[i].x) * static_cast<double>(p[j].x) + static_cast<double>(p[i].y) * static_cast<double>(p[j].y) + static_cast<double>(p[i].z) * static_cast<double>(p[j].z);
  }
...
#undef SetDelta
  ...
  */
  ...
  if ((delta[0][subset3] > 0.0) && (delta[1][subset3] > 0.0) && (delta[2][subset3] > 0.0))
  {
    //Yes
    return closest_face(delta[0][subset3], delta[1][subset3], delta[2][subset3], 0, 1, 2, p, index, cp);
  }
...
#undef TestDelta
...

  {
    ...
    for (int i = 0; (i < 2); i++)
    {
      for (int j = i + 1; (j < 3); j++)
      {
        if ((delta[i][subset2[i][j]] > 0.0) && (delta[j][subset2[i][j]] > 0.0))
        {
          double d = (delta[i][subset2[i][j]] * dp[i][i] + delta[j][subset2[i][j]] * dp[j][i]) / (delta[i][subset2[i][j]] + delta[j][subset2[i][j]]);

          if (d < dMin2)
          {
            dMin2 = d;
            nMin = closest_edge(delta[i][subset2[i][j]], delta[j][subset2[i][j]], i, j, p, index, cp);
          }
        }
      }
    }
...

  {
    if ((delta[0][subset3] > 0.0) && (delta[1][subset3] > 0.0) && (delta[2][subset3] > 0.0))
    {
      double d = (delta[0][subset3] * dp[0][0] + delta[1][subset3] * dp[1][0] + delta[2][subset3] * dp[2][0]) / (delta[0][subset3] + delta[1][subset3] + delta[2][subset3]);

      if (d < dMin2)
      {
        dMin2 = d;
        nMin = closest_face(delta[0][subset3], delta[1][subset3], delta[2][subset3], 0, 1, 2, p, index, cp);
      }
    }
  }
...
static int Johnson4(const Vector p[4], Vector *cp, int index[])
{
  //Mappings between all possible subsets of the vertices and a
  //unique subset ID.
  //  subset1[i]          ({Pi})
  ...
  //  subset4             ({P0, P1, P2, P3})
  static constexpr int subset1[4] = {0, 1, 2, 3};

  static constexpr int subset2[4][4] = {{-1, 4, 5, 6}, {4, -1, 7, 8}, {5, 7, -1, 9}, {6, 8, 9, -1}};
  static constexpr int subset3[4] = {10, 11, 12, 13};
  static constexpr int subset4 = 14;
...
#undef SetDelta
  ...
  //since there is no Pj not in S
  if ((delta[0][subset4] > 0.0) && (delta[1][subset4] > 0.0) && (delta[2][subset4] > 0.0) && (delta[3][subset4] > 0.0))
  {
    //Yes
    *cp = Vector::GetZero();
    return 4;
...
#undef TestDelta
...

  {
    {
      //Try the faces
      constexpr int ids[4][4] = {{0, 1, 2, 3}, {0, 1, 3, 2}, {0, 2, 3, 1}, {1, 2, 3, 0}};
      for (int f = 0; (f < 4); f++)
      {
        int i = ids[f][0];
        int j = ids[f][1];
        ...
        if ((delta[i][subset3[l]] > 0.0) && (delta[j][subset3[l]] > 0.0) && (delta[k][subset3[l]] > 0.0))
        {
          double d = (delta[i][subset3[l]] * dp[i][i] + delta[j][subset3[l]] * dp[j][i] + delta[k][subset3[l]] * dp[k][i]) / (delta[i][subset3[l]] + delta[j][subset3[l]] + delta[k][subset3[l]]);

          if (d < dMin2)
          {
            dMin2 = d;
            nMin = closest_face(delta[i][subset3[l]], delta[j][subset3[l]], delta[k][subset3[l]], i, j, k, p, index, cp);
          }
        }
      }
    }
...
int Johnson(int n, const Vector p[], Vector *cp, int index[])
{
  switch (n)
  {
    case 1:
    ...
    case 2: { return Johnson2(p, cp, index); }

    case 3: { return Johnson3(p, cp, index); }

    default: { return Johnson4(p, cp, index); }
  }
}

static bool DoGilbert(HitTest *phtHullA, HitTestShape htsA, const Vector &positionStart, const Vector &positionStop, const Vector &dV, const Orientation &orientationHullA, HitTest *phtHullB, HitTestShape htsB, Vector simplex[4])
{
  int nVertices = 3;//Always start with four (but the three is incremented before it is used).

  static constexpr int c_maxHistory = 100;
  ...
  while (true)
  {
    ...
    if (nMin == 4) { return true; }

    //Find a new extreme point
    {
      double l2 = cp.LengthSquared();
      if (l2 < epsilon * epsilon) return true;

      cp /= static_cast<float>(sqrt(l2));
    }

    Vector v0 = phtHullA->GetMinExtreme(htsA, (cp * dV >= 0.0f) ? positionStop : positionStart, orientationHullA, cp) - phtHullB->GetMaxExtreme(htsB, cp);
