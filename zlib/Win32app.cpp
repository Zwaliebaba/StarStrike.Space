...
void ZAssertImpl(bool bSucceeded, const char *psz, const char *pszFile, int line, const char *pszModule)
{
  if (!bSucceeded)
  {
    ...
    else if (g_papp->OnAssert(psz, pszFile, line, pszModule)) { g_papp->OnAssertBreak(); }
  }
}

void ZDebugOutputImpl(const char *psz)
{
  if (g_papp) g_papp->DebugOutput(psz);
...
void ZWarningImpl(bool bSucceeded, const char *psz, const char *pszFile, int line, const char *pszModule) { if (!bSucceeded) { debugf("%s(%d) : ShouldBe failed: '%s'\n", pszFile, line, psz); } }
...
#ifdef _TRACE
bool g_bEnableTrace = false; ZString g_strSpaces; int g_indent = 0; int g_line = 0; void SetStrSpaces() { g_strSpaces = " " + ZString((float) g_indent, 2, 0) + ZString(' ', g_indent * 2 + 1); } void ZTraceImpl(const char *pcc)
{
  if (g_bEnableTrace) { ZDebugOutput(ZString((float) g_line, 4, 0) + g_strSpaces + ZString(pcc) + "\n"); }
  g_line++;
...
void InitializeDebugf()
{
  ...
  {
    ::RegQueryValueEx(hKey, "LogToFile", nullptr, &dwType, (unsigned char *) &szValue, &cbValue);
    RegCloseKey(hKey);
  ...
  if (bLogToFile)
  {
    ...
    sprintf(p + 3, "%02d%02d%02d%02d.txt", t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);

    g_logfile = CreateFile(logFileName, GENERIC_WRITE, 0, nullptr, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, nullptr);
  }
#endif
}
...
Win32App::Win32App() { g_papp = this; }

Win32App::~Win32App() {}

HRESULT Win32App::Initialize(const ZString &strCommandLine) { return S_OK; }
...
int Win32App::OnException(DWORD code, ExceptionData *pdata) { return EXCEPTION_CONTINUE_SEARCH; }
...
{
#ifdef MemoryOutput
  g_listOutput.PushFront(ZString(psz)); if (g_listOutput.GetCount() > 100) { g_listOutput.PopEnd(); }
#else
  ...
  {
    DWORD nBytes;
    WriteFile(g_logfile, psz, strlen(psz), &nBytes, nullptr);
  }
#endif
}
...
{
  ZDebugOutput(ZString("assertion failed: '") + psz + "' (" + pszFile + ":" + ZString(line) + ")\n");
...
{
#ifdef MemoryOutput
  ZString str; TList<ZString>::Iterator iter(g_listOutput); while (!iter.End())
  {
    str = iter.Value() + str;
    iter.Next();
  }
...
{
  __try { return Win32MainInner(lpszCmdLine); }
  __except (g_papp->OnException(_exception_code(), static_cast<ExceptionData *>(_exception_info()))) { return 1; }
}
...
static int Win32MainInner(LPSTR lpszCmdLine)
{
  ...
  do
  {
    ...
    if (SUCCEEDED(hr) && S_FALSE != hr) { Window::MessageLoop(); }
...
__declspec(dllexport) int WINAPI Win32Main(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
  ...
  srand(GetTickCount() + time(nullptr));
