...
constexpr float AnyViewMinRenderScreenSize = 1.0f;

constexpr float c_dtFlashOn = 0.25f;
constexpr float c_dtFlashOff = 0.25f;
constexpr float c_dtFlashingDuration = 2.0f;

constexpr int c_nCountdownMax = 1000000;// just a big number
constexpr int c_nMinGain = -60;
...
namespace SoundInit
{
  void InitializeSoundTemplates(Modeler *pmodeler, TVector<TRef<ISoundTemplate>> &vSoundMap);
...
TRef<ZWriteFile> g_pzfFrameDump = nullptr;
FrameDumpInfo g_fdInfo;

constexpr float s_fCommandViewDistanceMax = 20000.0f;
constexpr float s_fCommandViewDistanceMin = 50.0f;
constexpr float s_fCommandViewDistanceDefault = 6000.0f;
constexpr float c_fCommandGridRadius = 7500.0f;

constexpr float s_fCommandViewDisplacementMax = 10000.0f;

constexpr float s_fExternalViewDistanceMax = 6000.0f;
constexpr float s_fExternalViewDistanceMin = 100.0f;
constexpr float s_fExteralViewDistanceDefault = 500.0f;

constexpr float c_dtRejectQueuedCommand = 15.0f;

constexpr float c_fVolumeDelta = 1;

constexpr float g_hudBright = 0.85f;

constexpr float g_fJoystickDeadZoneSmall = 0.1f;
constexpr float g_fJoystickDeadZoneLarge = 0.3f;
...
static constexpr unsigned char buttonDown = 0x01;
static constexpr unsigned char buttonChanged = 0x80;
...
{
  assert(pmodel);
  if (trekClient.GetShip()->GetParentShip() == nullptr)
  {
    float speed = pmodel->GetVelocity().Length();
    float myMaxSpeed = trekClient.GetShip()->GetHullType()->GetMaxSpeed();

    return (speed >= myMaxSpeed) ? 1.0f : ((2.0f * speed / myMaxSpeed) - 1.0f);
  }
  return 0.0f;
}
...
TRef<IMessageBox> CreateMessageBox(const ZString &str, ButtonPane *pbuttonIn, bool fAddDefaultButton, bool fAddCancelButton, float paintDelay)
{
  ...
  if (fAddDefaultButton)
  {
    ...
    pbutton = CreateTrekButton(CreateButtonFacePane(psurfaceButton, ButtonNormal, 0, psurfaceButton->GetSize().X()), false, positiveButtonClickSound);
  }
  ...
  if (fAddCancelButton)
  {
    ...
    pbuttonCancel = CreateTrekButton(CreateButtonFacePane(psurfaceButton, ButtonNormal, 0, psurfaceButton->GetSize().X()), false, negativeButtonClickSound);
  }
...
ImodelIGC *GetCurrentTarget(void)
{
  ...
  if (pmodelTarget != nullptr)
  {
    if (pmodelTarget->GetObjectType() == OT_ship) pmodelTarget = static_cast<IshipIGC *>(pmodelTarget)->GetSourceShip();

    if ((pmodelTarget == trekClient.GetShip()->GetSourceShip()) || (pmodelTarget->GetHitTest() == nullptr) || (pmodelTarget->GetCluster() != trekClient.GetCluster()) || !trekClient.GetShip()->CanSee(pmodelTarget)) pmodelTarget = nullptr;
  }
...
int GetSimilarTargetMask(ImodelIGC *pmodel)
{
  ...
  if ((type == OT_ship) || (type == OT_station) || (type == OT_probe)) { tt |= (trekClient.GetShip()->GetSide() == pmodel->GetSide()) ? c_ttFriendly : c_ttEnemy; }
  else tt |= c_ttAllSides;
...
public:
  CameraControl(void)
    : m_fov(s_fDefaultFOV),
      m_dtAnimate(0.0f),
      m_jiggle(0.0f),
      m_oldHeadYaw(0.0f) { m_orientationHead.Reset(); }
  ...
  const Orientation &GetHeadOrientation(void) const { return m_orientationHead; }
  ...
  void SetZClip(float zMin, float zMax) { m_pcamera->SetZClip(zMin, zMax); }
  ...
  bool Apply(float dt, bool bJiggle, bool bTarget)
  {
    ...
    if (m_dtAnimate > 0.0f)
    {
      ...
      if (tNext >= m_dtAnimate)
      {
        ...
        if (m_bAnimatePosition) m_position = m_position2;
      ...
      else
      {
        ...
        if (m_bAnimatePosition) { m_position = (m_position1 * (1.0f - interpolate)) + (m_position2 * interpolate); }
        m_orientation = Slerp(m_quaternion1, m_quaternion2, interpolate);
        m_fov = (m_fov1 * (1.0f - interpolate)) + (m_fov2 * interpolate);
    ...
    //
    constexpr double jiggleHalfLife = 0.25f;
    m_jiggle *= static_cast<float>(pow(jiggleHalfLife, static_cast<double>(dt)));
    ...
    if (bJiggle)
    {
      auto afterburner = static_cast<IafterburnerIGC *>(trekClient.GetShip()->GetSourceShip()->GetMountedPart(ET_Afterburner, 0));
      ...
      if (pmodelAleph)
      {
        float range = (pmodelAleph->GetPosition() - pshipSource->GetPosition()).Length() - trekClient.GetShip()->GetRadius();
    ...
    if (bTarget)
    {
      ...
      else m_pwrapGeoTarget->SetGeo(Geo::GetEmpty());
    }
  ...
  float GetAnimation(void) const { return m_dtAnimate; }
  ...
  {
    if (m_dtAnimate <= 0.0f) m_orientation = orientation;
    else m_quaternion2 = orientation;
  }
  ...
  const Orientation &GetOrientation(void) const { return m_orientation; }

  const Vector &GetCameraPosition(void) const
  {
    if (m_pcamera) return m_pcamera->GetPosition();
    return m_position;
  }

  const Orientation &GetCameraOrientation(void) const
  {
    if (m_pcamera) return m_pcamera->GetOrientation();
    return m_orientation;
  }

  void SetAnimatePosition(bool bAnimatePosition) { m_bAnimatePosition = bAnimatePosition; }
...
public:
  ...
  HRESULT GetUpDirection(Vector &vectUp) override
  {
  ...
  HRESULT GetPosition(Vector &vectPosition) override
  {
  ...
  HRESULT GetVelocity(Vector &vectVelocity) override
  {
    // REVIEW: the camera's velocity is not the same as the ships!
    vectVelocity = trekClient.GetShip()->GetCluster() ? trekClient.GetShip()->GetVelocity() : Vector(0, 0, 0);
    return S_OK;
  }

  // Gets the orientation of the sound in space, used for sound cones.
  HRESULT GetOrientation(Vector &vectOrientation) override
  {
  ...
  HRESULT IsListenerRelative() override
  {
  ...
  HRESULT IsPlaying() override { return S_OK; }
};
...
class TrekWindowImpl : public TrekWindow, public IIntegerEventSink, public ISubmenuEventSink, public IMenuCommandSink, public IClientEventSink, public ModelerSite, public TrekInputSite
{
public:
  ...
  public:
    ...
    bool OnChar(IInputProvider *pprovider, const KeyState &ks) override { return false; }

    bool OnKey(IInputProvider *pprovider, const KeyState &ks, bool &fForceTranslate) override { return m_pwindow->OnSuperKeyFilter(pprovider, ks, fForceTranslate); }
  ...
  {
    if (ks.bDown && m_pmenuQuickChat && ks.vk == VK_TAB && m_pconsoleImage != nullptr)
    {
      //
  ...
  public:
    ...
    bool OnKey(IInputProvider *pprovider, const KeyState &ks, bool &fForceTranslate) override { return m_pwindow->OnKeyFilter(pprovider, ks, fForceTranslate); }
  ...
  class QuickChatMenuCommandSink : public IMenuCommandSink, public ISubmenuEventSink
  {
    TrekWindowImpl *m_pwindow;
  ...
  public:
    ...
    void OnMenuCommand(IMenuItem *pitem) override { m_pwindow->OnQuickChatMenuCommand(m_pqcmenu, pitem); }

    void OnMenuSelect(IMenuItem *pitem) override {}

    void OnMenuClose(IMenu *pmenu) override { m_pwindow->OnQuickChatMenuClose(pmenu); }

    TRef<IPopup> GetSubMenu(IMenuItem *pitem) override { return m_pwindow->OnQuickChatMenuCommand(m_pqcmenu, pitem); }
  };
  ...
  void OnQuickChatMenuClose(IMenu *pmenu)
  {
    ...
    {
      m_pmenuQuickChat = nullptr;

      if (m_pconsoleImage && m_bQuitComposing) m_pconsoleImage->QuitComposing();
    }
  }
  ...
  TRef<IPopup> OnQuickChatMenuCommand(QuickChatMenu *pqcmenu, IMenuItem *pmenuItem)
  {
    ...
    auto pitem = static_cast<QuickChatMenuItem *>(plist->GetNth(pmenuItem->GetID()));
    ...
    if (pnode->GetType()->IsTypeOf(m_pmdlTypeMenu))
    {
      ...
      return CreateQuickChatMenu(static_cast<QuickChatMenu *>(pnode));
    }
    //
    // It's a command
    //

    auto pqcc = static_cast<QuickChatCommand *>(pnode);

    Training::CommandAcknowledgedCondition::SetCommandAcknowledged(static_cast<char>(pqcc->GetCommandID()));

    if (m_pconsoleImage) { m_pconsoleImage->SendQuickChat(pqcc->GetSonicChat()->GetIndex(), static_cast<int>(pqcc->GetTargetType()), static_cast<char>(pqcc->GetCommandID()), static_cast<short>(pqcc->GetAbilityMask())); }
    else
    {
      // if the console is not around, send it to the lobby chat recipient.
      trekClient.SendChat(trekClient.GetShip(), m_ctLobbyChat, NA, pqcc->GetSonicChat()->GetIndex(), nullptr);
    }
  ...
  void OpenMainQuickChatMenu() override
  {
    if (m_pconsoleImage) m_pconsoleImage->StartQuickChat(CHAT_NOSELECTION, NA, nullptr);
  ...
  TRef<IMenu> CreateQuickChatMenu(QuickChatMenu *pqcmenu)
  {
    ...
    TRef<IMenu> pmenu = CreateMenu(GetModeler(), TrekResources::SmallFont(), pmenuCommandSinkQuickChat);
    ...
    {
      auto pitem = static_cast<QuickChatMenuItem *>(plist->GetCurrent());

      QuickChatNode *pnode = pitem->GetNode();

      if (pnode->GetType()->IsTypeOf(m_pmdlTypeMenu)) { pmenu->AddMenuItem(index, pitem->GetString(), static_cast<char>(pitem->GetChar()), pmenuCommandSinkQuickChat); }
      else pmenu->AddMenuItem(index, pitem->GetString(), static_cast<char>(pitem->GetChar()));
  ...
  void InitializeQuickChatMenu()
  {
    ...
    CastTo(plist, pnsQuickChat->FindMember("sonicChats"));
    TLookup<SonicChat, DoSetIndex<SonicChat>>::Parse(plist, 0, m_vsonicChats);

    m_pqcmenuMain = static_cast<QuickChatMenu *>(pnsQuickChat->FindMember("mainMenu"));
  ...
  void PlaySonicChat(int id, int voice) override
  {
    if ((id >= 0) && (id < m_vsonicChats.GetCount()))
    {
      ...
      if (psonicChat)
      {
        ...
        if (plistVoices)
        {
          TRef<SonicChatVoice> pvoice = static_cast<SonicChatVoice *>(plistVoices->GetNth(voice));

          if (pvoice) StartSound(static_cast<ISoundTemplate *>(static_cast<IObject *>(pvoice->GetSound())));
          else trekClient.PlaySoundEffect(id);
        }
      }
    }
  }

  ZString GetSonicChatText(int id, int voice) override
  {
    if ((id >= 0) && (id < m_vsonicChats.GetCount()))
    {
      SonicChat *psonicChat = m_vsonicChats[id];
      if (psonicChat)
      {
        ...
        {
          TRef<SonicChatVoice> pvoice = static_cast<SonicChatVoice *>(plistVoices->GetNth(voice));
          if (pvoice) return pvoice->GetString();
        }
      }
    }
...
public:
  TRef<AnimatedImage> LoadAnimatedImage(Number *ptime, const ZString &str) override
  {
    TRef<AnimatedImage> pimage;

    if (m_mapAnimatedImages.Find(str, pimage)) return new AnimatedImage(ptime, pimage);
  ...
  public:
    ...
    GameStateCloseSink(TrekWindowImpl *pwindow, IEventSource *pevent)
      : m_pwindow(pwindow),
        m_pevent(pevent) { m_pevent->AddSink(this); }

    void Terminate() { m_pevent->RemoveSink(this); }
  ...
  void OnGameStateClose() { TurnOffOverlayFlags(ofGameState); }

  void InitializeGameStateContainer()
  {
    m_pgsc = m_pconsoleImage->GetGameStateContainer();
    m_pgameStateCloseSink = new GameStateCloseSink(this, m_pgsc->GetCloseEvent());
    ...
    if (pmp->IsConquestGame())
    {
      ...
      if (pmp->GetConquestPercentage() == 100) m_pstringPaneConquest->SetString("1st team to own all of the starbases wins");
      else { m_pstringPaneConquest->SetString("1st team to own " + ZString(pmp->GetConquestPercentage()) + "% of the starbases wins"); }
    }
    else m_pstringGridImageConquest = nullptr;
    ...
    if (pmp->IsTerritoryGame())
    {
      ...
      m_pstringPaneTerritory->SetString("1st team to own " + ZString((50 + trekClient.m_pCoreIGC->GetClusters()->n() * pmp->GetTerritoryPercentage()) / 100) + " sectors wins");
    }
    else m_pstringGridImageTerritory = nullptr;
    ...
    if (pmp->IsProsperityGame())
    {
      ...
      m_pstringPaneProsperity->SetString("First team to purchase reinforcements wins");
    }
    else m_pstringGridImageProsperity = nullptr;
    ...
    if (pmp->IsArtifactsGame())
    {
      ...
      m_pstringPaneArtifacts->SetString("First team to own " + ZString(pmp->nGoalArtifactsCount) + " artifacts wins");
    }
    else m_pstringGridImageArtifacts = nullptr;
    ...
    if (pmp->IsFlagsGame())
    {
      ...
      m_pstringPaneFlags->SetString("First team to capture " + ZString(pmp->nGoalFlagsCount) + " flags wins");
    }
    else m_pstringGridImageFlags = nullptr;

    {
      ...
      m_pstringPaneDeathMatch = m_pgsc->AddGameStateTile(pmp->IsDeathMatchGame() ? "wctitledeathmatchbmp" : "wctitledeathsbmp", ppaneGrid);
      m_pstringPaneDeathMatch->SetFont(TrekResources::SmallFont());
      m_pstringPaneDeathMatch->SetTextColor(Color::White());

      m_pstringPaneDeathMatch->SetString(pmp->IsDeathMatchGame() ? "1st team to kill " + ZString(pmp->GetDeathMatchKillLimit()) + " ships wins" : "");
    }
  ...
  void TerminateGameStateContainer()
  {
    ...
    if (m_pgameStateCloseSink)
    {
      ...
      m_pgameStateCloseSink = nullptr;
    }

    m_pstringPaneConquest = nullptr;
    m_pstringPaneTerritory = nullptr;
    m_pstringPaneDeathMatch = nullptr;
    m_pstringPaneCountdown = nullptr;
    m_pstringPaneProsperity = nullptr;
    m_pstringPaneArtifacts = nullptr;
    m_pstringPaneFlags = nullptr;

    m_pstringGridImageConquest = nullptr;
    m_pstringGridImageTerritory = nullptr;
    m_pstringGridImageDeathMatch = nullptr;
    m_pstringGridImageProsperity = nullptr;
    m_pstringGridImageArtifacts = nullptr;
    m_pstringGridImageFlags = nullptr;
  }
  ...
  {
  public:
    void ClearLine(StringGridImage *pgrid, int row) override { pgrid->SetString(row, 2, ZString()); }
  };

  class WinConditionInfo4 : public WinConditionInfo
  {
  public:
    void ClearLine(StringGridImage *pgrid, int row) override
    {
  ...
  {
  public:
    void FillLine(StringGridImage *pgrid, int row, IsideIGC *pside) override { pgrid->SetString(row, 2, ZString(pside->GetConquestPercent()) + "% owned"); }

    bool Greater(IsideIGC *pside1, IsideIGC *pside2) override { return pside1->GetConquestPercent() > pside2->GetConquestPercent(); }
  ...
  {
  public:
    void FillLine(StringGridImage *pgrid, int row, IsideIGC *pside) override
    {
      unsigned char c = pside->GetTerritoryCount();
      pgrid->SetString(row, 2, ZString(c) + ((c == 1) ? " sector owned" : " sectors owned"));
    }

    bool Greater(IsideIGC *pside1, IsideIGC *pside2) override { return pside1->GetTerritoryCount() > pside2->GetTerritoryCount(); }
  ...
  public:
    ...
    bool Greater(IsideIGC *pside1, IsideIGC *pside2) override
    {
      if (pside1->GetKills() == pside2->GetKills()) return pside1->GetDeaths() < pside2->GetDeaths();
  ...
  {
  public:
    void FillLine(StringGridImage *pgrid, int row, IshipIGC *pship) override
    {
    ...
    bool Greater(IshipIGC *pship1, IshipIGC *pship2) override
    {
      if (pship1->GetKills() == pship2->GetKills()) return pship1->GetDeaths() < pship2->GetDeaths();
      return pship1->GetKills() > pship2->GetKills();
    }

    bool AnyInfo() override { return true; }
  };
  ...
  public:
    void FillLine(StringGridImage *pgrid, int row, IsideIGC *pside) override
    {
      ...
      if (pbl)
      {
        ...
        if ((pbuyable->GetObjectType() == OT_development) && (pbuyable->GetObjectID() == c_didTeamMoney)) prosperity = pbl->data()->GetPercentComplete();
      }

      pgrid->SetString(row, 2, ZString(prosperity) + "% invested");
    }
  ...
  {
  public:
    void FillLine(StringGridImage *pgrid, int row, IsideIGC *pside) override { pgrid->SetString(row, 2, ZString(pside->GetArtifacts()) + " owned"); }

    bool Greater(IsideIGC *pside1, IsideIGC *pside2) override { return pside1->GetArtifacts() > pside2->GetArtifacts(); }
  ...
  {
  public:
    void FillLine(StringGridImage *pgrid, int row, IsideIGC *pside) override { pgrid->SetString(row, 2, ZString(pside->GetFlags()) + " captured"); }

    bool Greater(IsideIGC *pside1, IsideIGC *pside2) override { return pside1->GetFlags() > pside2->GetFlags(); }
  ...
  void FillWinCondition(WinConditionInfo &info, ShipWinConditionInfo &shipInfo, StringGridImage *pgrid)
  {
    // top team
    // my team
    // top player
    ...
    {
      IsideIGC *psideTop = nullptr;
      int myRank = 1;

      for (SideLinkIGC *psl = trekClient.m_pCoreIGC->GetSides()->first(); (psl != nullptr); psl = psl->next())
      {
        IsideIGC *pside = psl->data();

        if (pside != psideMe)
        {
          if ((psideTop == nullptr) || info.Greater(pside, psideTop)) psideTop = pside;

          if (info.Greater(pside, psideMe)) myRank++;
        }
      }
    ...
    if (shipInfo.AnyInfo())
    {
      IshipIGC *pshipTop = nullptr;
      int myRank = 1;

      for (ShipLinkIGC *psl = trekClient.m_pCoreIGC->GetShips()->first(); (psl != nullptr); psl = psl->next())
      {
        IshipIGC *pship = psl->data();

        if ((pship != pshipMe) && (pship->GetPilotType() >= c_ptPlayer))
        {
          if ((pshipTop == nullptr) || shipInfo.Greater(pship, pshipTop)) pshipTop = pship;

          if (shipInfo.Greater(pship, pshipMe)) myRank++;
        }
      }
  ...
  void UpdateGameStateContainer() override
  {
    if (m_pgsc)
    {
      if (m_pstringGridImageConquest) { FillWinCondition(s_conquestWinConditionInfo, s_shipWinConditionInfo, m_pstringGridImageConquest); }

      if (m_pstringGridImageTerritory) { FillWinCondition(s_territoryWinConditionInfo, s_shipWinConditionInfo, m_pstringGridImageTerritory); }

      if (m_pstringGridImageDeathMatch) { FillWinCondition(s_deathMatchWinConditionInfo, s_deathMatchShipWinConditionInfo, m_pstringGridImageDeathMatch); }

      if (m_pstringGridImageProsperity) { FillWinCondition(s_prosperityWinConditionInfo, s_shipWinConditionInfo, m_pstringGridImageProsperity); }

      if (m_pstringGridImageArtifacts) { FillWinCondition(s_artifactsWinConditionInfo, s_shipWinConditionInfo, m_pstringGridImageArtifacts); }
      if (m_pstringGridImageFlags) { FillWinCondition(s_flagsWinConditionInfo, s_shipWinConditionInfo, m_pstringGridImageFlags); }
    }
  }
  ...
  void InitializeSoundTemplates() { SoundInit::InitializeSoundTemplates(GetModeler(), m_vSoundMap); }
  ...
  bool GetUI() { return m_screen != ScreenIDCombat; }

  void SetScreen(Screen *pscreen)
  {
    m_pimageScreen = pscreen->GetImage();

    if (m_pimageScreen == nullptr)
    {
  ...
  ScreenID screen(void) const override { return m_screen; }

  // review: this gets messy because it can be recursively called by a screen's
  // constructor.

  void screen(ScreenID s) override { screen(s, -1); }

  void screen(ScreenID s, int idPlayer, const char *szPlayerName = nullptr, const char *szOther = nullptr)
  {
    ...
    if (s != m_screen)
    {
      // Hold old screen/image alive until end of block so shared
      // resources (Device3D, etc.) are not prematurely destroyed
      // before the new screen is created.
      TRef<Image> pimageScreenOld;
      TRef<Screen> pscreenOld;
      ...
      if (m_pimageScreen != nullptr)
      {
        pimageScreenOld = m_pimageScreen;
        pscreenOld = m_pscreen;
        m_pwrapImageTop->SetImage(m_pgroupImageGame);
        m_pimageScreen = nullptr;
        m_pscreen = nullptr;
        SetCaption(nullptr);
      }
      ...
      if (s != ScreenIDCombat)
      {
        if (GetViewMode() == vmOverride) EndOverrideCamera();
        else SetCameraMode(cmCockpit);

        m_pwrapImageConsole->SetImage(Image::GetEmpty());
        m_pconsoleImage = nullptr;
        TerminateGameStateContainer();
        trekClient.RequestViewCluster(nullptr);
        m_pscreenBackdrop = nullptr;
        m_pimageBackdrop = nullptr;
        m_viewmode = vmUI;
      }
      ...
      if (!GetPopupContainer()->IsEmpty()) GetPopupContainer()->ClosePopup(nullptr);
      m_pmessageBox = nullptr;
      m_pmessageBoxLockdown = nullptr;
      ...
      switch (s)
      {
        case ScreenIDCombat:
        {
          ...
          SetViewMode(trekClient.GetShip()->GetStation() ? (trekClient.GetShip()->IsGhost() ? vmCommand : vmHangar) : vmCombat, true);
          PositionCommandView(nullptr, 0.0f);
        ...
        {
          TRef<Screen> pscreen = CreateTrainingScreen(GetModeler());
          if (pscreen == nullptr)
          {
            TRef<IMessageBox> pmsgBox = CreateMessageBox("Training is not available in this build.", nullptr, true);
            GetPopupContainer()->OpenPopup(pmsgBox, false);
          }
          else SetScreen(pscreen);
          break;
        }
        ...
        case ScreenIDSquadsScreen:
          SetScreen(CreateSquadsScreen(GetModeler(), szPlayerName ? static_cast<PCC>(strPlayerName) : nullptr, idPlayer, szOther ? static_cast<PCC>(strOther) : nullptr));
          break;
        ...
        case ScreenIDCharInfo:
          if (idPlayer == -1) idPlayer = trekClient.GetZoneClubID();
        ...
        case ScreenIDTrainSlideshow:
        {
          ...
          char *strNamespace[] = {"", "", "tm_2_basic_flight", "tm_3_basic_weaponry", "tm_4_enemy_engagement", "tm_5_command_view", "tm_6_practice_arena",};
        ...
        case ScreenIDPostTrainSlideshow:
        {
          ...
          int iMission = static_cast<int>(g_pnumberMissionNumber->GetValue());
          ZAssert((iMission >= 1) && (iMission <= 6));
          char *strNamespace[] = {"", "tm_1_introduction", "tm_2_basic_flight_post", "tm_3_basic_weaponry_post", "tm_4_enemy_engagement_post", "tm_5_command_view_post", "tm_6_practice_arena_post",};
    ...
    if (s != sNextScreen) screen(sNextScreen, NULL);
  }

  void CharInfoScreenForPlayer(int idZone) override { screen(ScreenIDCharInfo, idZone); }

  void SquadScreenForPlayer(const char *szName, int idZone, const char *szSquad) override { screen(ScreenIDSquadsScreen, idZone, szName, szSquad); }

  void LeaderBoardScreenForPlayer(const ZString &strCharacter) override { screen(ScreenIDLeaderBoard, -1, strCharacter); }
  ...
  bool IsInputEnabled(int vk) { return (m_screen != ScreenIDCombat) || IsProbablyNotForChat(vk) || !m_pconsoleImage->IsComposing(); }

  TrekWindowImpl(EffectApp *papp, const ZString &strCommandLine, bool bMovies, bool bSoftware, bool bHardware, bool bPrimary, bool bSecondary)
    : TrekWindow(papp, strCommandLine, false, WinRect(0, 0, 800, 600), WinPoint(640, 480)),
      ...
      m_aabmCommand(0)
  {
    ...
    auto hIcon = static_cast<HICON>(::LoadImage(GetModuleHandle(nullptr), pszRes, IMAGE_ICON, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR));
    SendMessage(WM_SETICON, ICON_BIG, (LPARAM) hIcon);

    auto hIconSmall = static_cast<HICON>(::LoadImage(GetModuleHandle(nullptr), pszRes, IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR));
    ...
    if (FAILED(hr)) CreateDummyDiskPlayer(m_pDiskPlayer);
    ...
    TRef<INameSpace> pnsGamePanes = GetModeler()->CreateNameSpace("gamepanes", pnsSound);
    ...
    pnsGamePanes->AddMember("StyleHUD", m_pwrapNumberStyleHUD = new WrapNumber(m_pnumberStyleHUD = new ModifiableNumber(0)));
    ...
    pnsGamePanes->AddMember("SFXGain", m_pnumSFXGain = new ModifiableNumber(-static_cast<float>(LoadPreference("SFXGain", 5))));
    pnsGamePanes->AddMember("MusicGain", m_pnumMusicGain = new ModifiableNumber(-static_cast<float>(LoadPreference("MusicGain", 30))));
    pnsGamePanes->AddMember("VoiceOverGain", m_pnumVoiceOverGain = new ModifiableNumber(-static_cast<float>(LoadPreference("VoiceOverGain", 3))));
    ...
    m_cameraControl.m_pwrapGeoTarget = new WrapGeo(m_cameraControl.m_pgeoTarget), m_cameraControl.m_porientTransformTarget = new MatrixTransform(Matrix::GetIdentity());

    // initialize for chase view
    m_turnRateSampleIndex = 0;

    pnsGamePanes->AddMember("targetGeo", new TransformGeo(m_cameraControl.m_pwrapGeoTarget, m_cameraControl.m_porientTransformTarget));
    ...
    m_soundquality = static_cast<ISoundEngine::Quality>(LoadPreference("SoundQuality", ISoundEngine::midQuality));
    ...
    m_sizeCombat = WinPoint(static_cast<int>(LoadPreference("CombatXSize", 800)), static_cast<int>(LoadPreference("CombatYSize", 600)));

    m_sizeCombatFullscreen = WinPoint(static_cast<int>(LoadPreference("CombatFullscreenXSize", 800)), static_cast<int>(LoadPreference("CombatFullscreenYSize", 600)));

    //
    // Music toggle
    //

    if (LoadPreference("Music", TRUE)) ToggleMusic();
    ...
    m_cameraControl.SetCameras(m_pcamera, m_pcameraPosters/*,
            m_pcameraTurret*/
        );
    ...
    m_pimageScene = new GeoImage(GroupGeo::Create(m_pwrapGeoDebris = new WrapGeo(m_pgeoDebris), m_pgeoScene, m_pCommandVisibleGeo), m_pviewport, true);
    ...
    m_pimageLOD = CreatePaneImage(GetEngine(), SurfaceType2D(), false, ppane);
    ...
    m_pwrapImagePosters = new WrapImage(m_pwrapImagePostersInside = new WrapImage(Image::GetEmpty()));
    ...
    m_ptrekInput = CreateTrekInput(GetModuleHandle(nullptr), GetHWND(), GetInputEngine(), m_pjoystickImage);
    ...
    m_cVTVersion = 0;
    m_hwndVTEdit = ::CreateWindow(TEXT("EDIT"), TEXT("VTEdit"), 0, 0, 0, 0, 0, TrekWindow::GetHWND(), NULL, GetModuleHandle(NULL), NULL);

    //
    // Load saved settings
    //

    if (LoadPreference("RoundRadarScreen", FALSE)) ToggleRoundRadar();
    if (!LoadPreference("CensorChats", TRUE)) ToggleCensorChats();
    if (LoadPreference("PreferChaseView", FALSE)) ToggleStickyChase();
    if (!LoadPreference("FilterChatsToAll", TRUE)) ToggleFilterChatsToAll();
    if (!LoadPreference("FilterQuickComms", TRUE)) ToggleFilterQuickComms();
    if (!LoadPreference("FilterLobbyChats", TRUE)) ToggleFilterLobbyChats();
    if (!LoadPreference("LinearControlResponse", TRUE)) ToggleLinearControls();
    if (!LoadPreference("Environment", TRUE)) ToggleEnvironment();
    if (!LoadPreference("Posters", TRUE)) TogglePosters();
    if (!LoadPreference("Debris", TRUE)) ToggleDebris();
    if (!LoadPreference("Stars", TRUE)) ToggleStars();
    if (!LoadPreference("Strobes", TRUE)) ToggleStrobes();
    if (!LoadPreference("Trails", TRUE)) ToggleTrails();
    if (!LoadPreference("Bounds", TRUE)) ToggleBounds();
    if (!LoadPreference("TransparentObjects", TRUE)) ToggleTransparentObjects();
    SetSmoke(LoadPreference("SmokeEffects", 3));
    if (!LoadPreference("Lens Flare", TRUE)) ToggleLensFlare();
    if (!LoadPreference("BidirectionalLighting", TRUE)) ToggleBidirectionalLighting();
    if (!LoadPreference("ChatHistory", TRUE)) ToggleChatHistoryHUD();
    if (!LoadPreference("CenterHUD", TRUE)) ToggleCenterHUD();
    if (!LoadPreference("TargetHUD", TRUE)) ToggleTargetHUD();
    if (LoadPreference("SoftwareHUD", FALSE))//All we need with two styles
      CycleStyleHUD();
    if (LoadPreference("LargeDeadZone", FALSE)) ToggleLargeDeadZone();
    ...
    else bAllow3DAcceleration = LoadPreference("Allow3DAcceleration", TRUE) != 0;
    ...
    else bAllowSecondary = LoadPreference("AllowSecondary", TRUE) != 0;
    ...
    m_pmissileLast = nullptr;

    //
    // Show the intro videos
    //

    if (!g_bQuickstart && bMovies && !g_bReloaded) SetScreen(CreateVideoScreen(GetModeler(), false));
    else
    {
      SetScreen(CreateIntroScreen(GetModeler()));
  ...
  void SetCursor(const char *pszCursor) override
  {
    if (m_pszCursor != pszCursor)//Assume they are all static strings that are combined so pointers are all we need
    {
      ...
      if (GetCursorImage() != Image::GetEmpty()) RestoreCursor();
    }
  }
  ...
  void RestoreCursor() override
  {
    ...
    CastTo(pimageCursor, static_cast<Value *>(GetModeler()->GetNameSpace("cursor")->FindMember(m_pszCursor)));
    assert(pimageCursor);
  ...
  {
    TRef<Image> pimageCursor;
    CastTo(pimageCursor, static_cast<Value *>(GetModeler()->GetNameSpace("cursor")->FindMember(AWF_CURSOR_WAIT)));
    assert(pimageCursor);
    SetCursorImage(pimageCursor);
  }
  ...
  void SavePreference(const ZString &szName, DWORD dwValue)
  {
    ...
    if (ERROR_SUCCESS == ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &hKey, nullptr))
    {
  ...
  DWORD LoadPreference(const ZString &szName, DWORD dwDefault)
  {
    ...
    if (ERROR_SUCCESS == ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, "", REG_OPTION_NON_VOLATILE, KEY_READ, nullptr, &hKey, nullptr))
    {
      ...
      if (dwType != REG_DWORD) dwResult = dwDefault;
    }
  ...
  void SavePreference(const ZString &szName, const ZString &strValue)
  {
    ...
    if (ERROR_SUCCESS == ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &hKey, nullptr))
    {
      ::RegSetValueEx(hKey, szName, NULL, REG_SZ, (const unsigned char *) static_cast<const char *>(strValue), strValue.GetLength() + 1);
      RegCloseKey(hKey);
    }
  }
  ...
  ZString LoadPreference(const ZString &szName, const ZString &strDefault)
  {
    ...
    if (ERROR_SUCCESS == ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, "", REG_OPTION_NON_VOLATILE, KEY_READ, nullptr, &hKey, nullptr))
    {
      constexpr int nMaxStrLen = 2048;
      DWORD dwSize = nMaxStrLen;
      DWORD dwType = REG_SZ;
      unsigned char cbValue[nMaxStrLen + 1];

      ::RegQueryValueEx(hKey, szName, nullptr, &dwType, cbValue, &dwSize);
      RegCloseKey(hKey);

      cbValue[nMaxStrLen] = '\0';

      if (dwType == REG_SZ) strResult = cbValue;
    }
  ...
  void Terminate() override
  {
    ...
    // Terminate VT.
    //
    if (nullptr != m_hwndVTEdit) { if (TRUE == DestroyWindow(m_hwndVTEdit)) m_hwndVTEdit = nullptr; }
    ...
    m_pgeoScene = nullptr;
    m_pcamera = nullptr;
    m_pcameraPosters = nullptr;
    //m_pcameraTurret      = NULL;
    m_pimageScene = nullptr;
    m_pgeoDebris = nullptr;
    m_pimageStars = nullptr;
    m_pimageEnvironment = nullptr;
    m_pimageBlack = nullptr;
    //m_pimageTurret       = NULL;
    m_pimageLensFlare = nullptr;
    m_pwrapImageHudGroup = nullptr;
    m_pwrapImageComm = nullptr;
    m_pwrapImageRadar = nullptr;
    m_pwrapImageConsole = nullptr;
    m_pgroupImageHUD = nullptr;
    m_pconsoleImage = nullptr;
    m_pradarImage = nullptr;
    m_pgroupImage = nullptr;
    m_pgroupImage3D = nullptr;
    m_pconsoleImage = nullptr;

    m_psoundMusic = nullptr;
    m_vSoundMap.SetEmpty();
    m_vsonicChats.SetEmpty();
    m_pqcmenuMain = nullptr;
    m_psoundmutexSal->Reset();
    m_psoundmutexVO->Reset();
    m_pSoundEngine = nullptr;

    m_ptrekInput = nullptr;
  }
  ...
  void CloseMessageBox() { m_pmessageBox = nullptr; }
  ...
  public:
    ...
    {
      if (value == IDOK) m_pwindow->DoClose();
      else m_pwindow->CloseMessageBox();
      return false;
    }
  };
  ...
  void StartClose() override
  {
    ...
    {
      m_pmessageBox = CreateMessageBox("Quit Allegiance?", nullptr, true, true);
    ...
    if (!GetFullscreen()) OnCaptionRestore();
  }

  bool OnEvent(IIntegerEventSource *pevent, int value) override
  {
    if (pevent == m_peventLOD) ThingGeo::SetLODBias(static_cast<float>(value) / 100);
    return true;
  }
  ...
  void DoQuitMission()
  {
    ...
    else if (Slideshow::IsInSlideShow()) screen(ScreenIDTrainScreen);
    else
    {
      trekClient.QuitMission();

      SetWaitCursor();
      TRef<IMessageBox> pmsgBox = CreateMessageBox("Quitting team...", nullptr, false, false, 1.0f);
      GetPopupContainer()->OpenPopup(pmsgBox, false);
    }
  }
  ...
  public:
    ...
    bool OnEvent(IIntegerEventSource *pevent, int value) override
    {
      if (value == IDOK) m_pwindow->DoQuitMission();
      return false;
    }
  };

  void StartQuitMission() override
  {
    TRef<IMessageBox> pmessageBox;
    if (Training::IsTraining() || Slideshow::IsInSlideShow()) pmessageBox = CreateMessageBox("Quit the Current Training Mission?", nullptr, true, true);
    else pmessageBox = CreateMessageBox("Quit the Current Game?", nullptr, true, true);
    pmessageBox->GetEventSource()->AddSink(new QuitMissionSink(this));
    GetPopupContainer()->OpenPopup(pmessageBox, false);
  }

  TrekInput *GetInput(void) override { return m_ptrekInput; }

  void SetLightDirection(const Vector &direction) override
  {
    m_pimageLensFlare->SetLightDirection(direction);
    m_pimageScene->SetLightDirection(direction);
  }

  void SetCluster(IclusterIGC *pcluster) override
  {
    if (pcluster)
    {
      ...
      m_pimageEnvironment->SetGeo(CreateCopyModeGeo(pcluster->GetClusterSite()->GetEnvironmentGeo()));
  ...
  void UpdateBackdropCentering()
  {
    if (m_pimageBackdrop)
    {
      ...
      Point pntOffset((rectScreen.XSize() - sizePane.X()) / 2, (rectScreen.YSize() - sizePane.Y()) / 2);

      m_pimageBackdrop->SetTranslation(pntOffset);
    }
  }
  ...
  void Error(const ZString &str) override
  {
    SetFullscreen(false);
  ...
  Orientation GetCameraOrientation() override { return m_cameraControl.GetHeadOrientation() * m_cameraControl.GetOrientation(); }
  void ResetCameraFOV() override { m_cameraControl.SetFOV(s_fDefaultFOV); }
  Camera *GetCameraPosters() override { return m_pcameraPosters; }
  ConsoleImage *GetConsoleImage() override { return m_pconsoleImage; }
  Viewport *GetViewport() override { return m_pviewport; }
  Viewport *GetPosterViewport() override { return m_pviewportPosters; }
  int GetRadarMode(void) const override { return m_pradarImage->GetRadarLOD(); }

  void SetRadarMode(int radarMode) override
  {
    m_pradarImage->SetRadarLOD(static_cast<RadarImage::RadarLOD>(radarMode));
    m_radarCockpit = radarMode;
  }

  AsteroidAbilityBitMask GetInvestAsteroid(void) const override { return m_aabmInvest; }
  void SetInvestAsteroid(AsteroidAbilityBitMask aabm) override { m_aabmInvest = aabm; }

  AsteroidAbilityBitMask GetCommandAsteroid(void) const override { return m_aabmCommand; }
  void SetCommandAsteroid(AsteroidAbilityBitMask aabm) override { m_aabmCommand = aabm; }

  bool GetRoundRadarMode(void) const override { return m_bRoundRadar; }
  CameraMode GetCameraMode(void) const override { return m_cm; }
  ...
  */
  ...
  bool IsValid() override { return EngineWindow::IsValid(); }

  void ShowWebPage(const char *szURL) override
  {
    if (szURL[0] == '\0') szURL = "http://www.zone.com/allegiance";
...
#define idmVoiceOverVolumeDown  710
  ...
  void ShowMainMenu()
  {
    m_pmenu = CreateMenu(GetModeler(), TrekResources::SmallFont(), m_pmenuCommandSink);
    ...
    if (trekClient.MyMission() != nullptr) m_pmenu->AddMenuItem(idmGameDetails, "Game Details", 'I');

    m_pmenu->AddMenuItem(idmHelp, "Help", 'H');
    if ((trekClient.MyMission() != nullptr) || Slideshow::IsInSlideShow()) m_pmenu->AddMenuItem(idmExitGame, "Quit Mission", 'Q');
  ...
  TRef<IPopup> GetSubMenu(IMenuItem *pitem) override
  {
    TRef<IMenu> pmenu = CreateMenu(GetModeler(), TrekResources::SmallFont(), m_pmenuCommandSink);
    ...
    switch (pitem->GetID())
    {
      ...
      case idmSoundOptions:
        ...
        m_pitemMusicVolumeUp = pmenu->AddMenuItem(idmMusicVolumeUp, GetGainMenuString("Music", m_pnumMusicGain->GetValue(), c_fVolumeDelta), 'M');
        m_pitemMusicVolumeDown = pmenu->AddMenuItem(idmMusicVolumeDown, GetGainMenuString("Music", m_pnumMusicGain->GetValue(), -c_fVolumeDelta), 'N');
        m_pitemSFXVolumeUp = pmenu->AddMenuItem(idmSFXVolumeUp, GetGainMenuString("Sound Effect", m_pnumSFXGain->GetValue(), c_fVolumeDelta), 'S');
        m_pitemSFXVolumeDown = pmenu->AddMenuItem(idmSFXVolumeDown, GetGainMenuString("Sound Effect", m_pnumSFXGain->GetValue(), -c_fVolumeDelta), 'A');
        m_pitemVoiceOverVolumeUp = pmenu->AddMenuItem(idmVoiceOverVolumeUp, GetGainMenuString("Voice Over", m_pnumVoiceOverGain->GetValue(), c_fVolumeDelta), 'V');
        m_pitemVoiceOverVolumeDown = pmenu->AddMenuItem(idmVoiceOverVolumeDown, GetGainMenuString("Voice Over", m_pnumVoiceOverGain->GetValue(), -c_fVolumeDelta), 'C');
        break;
    }
  ...
  void ToggleDebris()
  {
    ...
    if (m_pitemToggleDebris != nullptr) m_pitemToggleDebris->SetString(GetDebrisMenuString());
  }
  ...
  void ToggleEnvironment()
  {
    ...
    if (m_pitemToggleEnvironment != nullptr) m_pitemToggleEnvironment->SetString(GetEnvironmentMenuString());
  }
  ...
  void ToggleRoundRadar()
  {
    ...
    if (m_pitemToggleRoundRadar != nullptr) m_pitemToggleRoundRadar->SetString(GetRoundRadarMenuString());
  }

  void ToggleCensorChats()
  {
    if (CensorDisplay()) SavePreference("CensorChats", FALSE);
    else SavePreference("CensorChats", TRUE);

    ToggleCensorDisplay();

    if (m_pitemToggleCensorChats != nullptr) m_pitemToggleCensorChats->SetString(GetCensorChatsMenuString());
  }
  ...
  void ToggleStickyChase()
  {
    ...
    if (m_pitemToggleStickyChase != nullptr) m_pitemToggleStickyChase->SetString(GetStickyChaseMenuString());
  }
  ...
  void ToggleFilterChatsToAll()
  {
    ...
    if (m_pitemFilterChatsToAll != nullptr) m_pitemFilterChatsToAll->SetString(GetFilterChatsToAllMenuString());
  }
  ...
  void ToggleFilterQuickComms()
  {
    ...
    if (m_pitemFilterQuickComms != nullptr) m_pitemFilterQuickComms->SetString(GetFilterQuickCommsMenuString());
  }
  ...
  void ToggleFilterLobbyChats()
  {
    ...
    if (m_pitemFilterLobbyChats != nullptr) m_pitemFilterLobbyChats->SetString(GetFilterLobbyChatsMenuString());
  }
  ...
  void ToggleLinearControls()
  {
    ...
    if (m_pitemToggleLinearControls != nullptr) m_pitemToggleLinearControls->SetString(GetLinearControlsMenuString());
  }
  ...
  void ToggleStars()
  {
    ...
    if (m_pitemToggleStars != nullptr) m_pitemToggleStars->SetString(GetStarsMenuString());
  }
  ...
  void TogglePosters()
  {
    ...
    if (m_pitemTogglePosters != nullptr) m_pitemTogglePosters->SetString(GetPostersMenuString());
  }
  ...
  void ToggleLensFlare()
  {
    ...
    SavePreference("Lens Flare", m_bLensFlare);

    if (!CommandCamera(m_cm))
    {
      if (m_bLensFlare) m_pwrapImageLensFlare->SetImage(m_pimageLensFlare);
      else m_pwrapImageLensFlare->SetImage(Image::GetEmpty());
    }

    if (m_pitemToggleLensFlare != nullptr) m_pitemToggleLensFlare->SetString(GetLensFlareMenuString());
  }
  ...
  void ToggleBidirectionalLighting()
  {
    ...
    SavePreference("BidirectionalLighting", m_bBidirectionalLighting);

    UpdateBidirectionalLighting();

    if (m_pitemToggleBidirectionalLighting != nullptr) m_pitemToggleBidirectionalLighting->SetString(GetBidirectionalLightingMenuString());
  }

  SoundID GetFlightMusic()
  {
    int nGrooveLevel = trekClient.GetGrooveLevel();
    static SoundID idLastFlightMusic = (random(0, 1) > 0.5) ? flightMusic1ASound : flightMusic2ASound;
    SoundID idNextFlightAMusic = (idLastFlightMusic < flightMusic2ASound) ? flightMusic2ASound : flightMusic1ASound;
    SoundID musicIdNew;

    if (idLastFlightMusic < flightMusic2ASound) idNextFlightAMusic = flightMusic2ASound;
    else if (idLastFlightMusic < flightMusic3ASound) idNextFlightAMusic = flightMusic3ASound;
    else idNextFlightAMusic = flightMusic1ASound;
    ...
    switch (m_musicId)
    {
      ...
      case flightMusic3CSound:
        if (nGrooveLevel == 2)
        {
          if (m_psoundMusic && m_psoundMusic->IsPlaying() == S_OK) musicIdNew = m_musicId;
          else musicIdNew = idNextFlightAMusic + 2;
        }
        else if (nGrooveLevel == 1) musicIdNew = m_musicId - 1;
        else musicIdNew = idNextFlightAMusic;
        break;

      case flightMusic1BSound:
      case flightMusic2BSound:
      case flightMusic3BSound:
        if (nGrooveLevel == 2) musicIdNew = m_musicId + 1;
        else if (nGrooveLevel == 1)
        {
          if (m_psoundMusic && m_psoundMusic->IsPlaying() == S_OK) musicIdNew = m_musicId;
          else musicIdNew = idNextFlightAMusic + 1;
        }
        else musicIdNew = idNextFlightAMusic;
        break;
      ...
      case flightMusic3ASound:
        if (nGrooveLevel == 2) musicIdNew = m_musicId + 2;
        else if (nGrooveLevel == 1) musicIdNew = m_musicId + 1;
        else
        {
          if (m_psoundMusic && m_psoundMusic->IsPlaying() == S_OK) musicIdNew = m_musicId;
      ...
      {
        if (m_psoundMusic && m_psoundMusic && m_psoundMusic->IsPlaying() == S_OK) return deathMusicSound;
  ...
  void UpdateMusic()
  {
    ...
    if (m_bMusic)
    {
      switch (m_screen)
      {
        ...
        default:
          if ((m_musicId == gameOverWonMusicSound || m_musicId == gameOverLostMusicSound) && m_psoundMusic && m_psoundMusic->IsPlaying() == S_OK) newMusicSound = m_musicId;
          else newMusicSound = gameScreenMusicSound;
          break;
      }
    }
  ...
  void TriggerMusic(SoundID newMusicSound) override
  {
    if (newMusicSound != m_musicId || m_psoundMusic == nullptr || m_psoundMusic->IsPlaying() == S_FALSE)
    {
      if (m_psoundMusic != nullptr) m_psoundMusic->Stop(true);

      if (newMusicSound != NA) m_psoundMusic = StartSound(newMusicSound);
      else m_psoundMusic = nullptr;

      m_musicId = newMusicSound;
    }
  }

  bool GetMusicIsOn(void) override { return m_bMusic; }

  void SetMusicOn(bool bMusicOn) override
  {
    m_bMusic = bMusicOn;
    UpdateMusic();
  }
  ...
  void ToggleStrobes()
  {
    ...
    SavePreference("Strobes", ThingGeo::GetShowLights());

    if (m_pitemToggleStrobes != nullptr) m_pitemToggleStrobes->SetString(GetStrobesMenuString());
  }
  ...
  void ToggleTrails()
  {
    ...
    SavePreference("Trails", ThingGeo::GetShowTrails());

    if (m_pitemToggleTrails != nullptr) m_pitemToggleTrails->SetString(GetTrailsMenuString());
  }
  ...
  void ToggleBounds()
  {
    ...
    SavePreference("Bounds", ThingGeo::GetShowBounds());

    if (m_pitemToggleBounds != nullptr) m_pitemToggleBounds->SetString(GetBoundsMenuString());
  }
  ...
  void ToggleTransparentObjects()
  {
    ...
    SavePreference("TransparentObjects", ThingGeo::GetTransparentObjects());

    if (m_pitemToggleTransparentObjects != nullptr) m_pitemToggleTransparentObjects->SetString(GetTransparentObjectsMenuString());
  }

  void SetSmoke(DWORD value) { ThingGeo::SetShowSmoke(static_cast<int>(value)); }
  ...
  void ToggleSmoke()
  {
    ...
    SavePreference("SmokeEffects", static_cast<DWORD>(iSmoke));

    if (m_pitemToggleSmoke != nullptr) m_pitemToggleSmoke->SetString(GetSmokeMenuString());
  }
  ...
  void ToggleChatHistoryHUD()
  {
    ...
    if (m_pitemToggleChatHistoryHUD != nullptr) m_pitemToggleChatHistoryHUD->SetString(GetChatHistoryHUDMenuString());
  }

  void ToggleCenterHUD()
  {
    m_pboolCenterHUD->SetValue(!m_pboolCenterHUD->GetValue());
    SavePreference("CenterHUD", m_pboolCenterHUD->GetValue());

    if (m_pitemToggleCenterHUD != nullptr) m_pitemToggleCenterHUD->SetString(GetCenterHUDMenuString());
  }

  void ToggleTargetHUD()
  {
    m_pboolTargetHUD->SetValue(!m_pboolTargetHUD->GetValue());
    SavePreference("TargetHUD", m_pboolTargetHUD->GetValue());

    if (m_pitemToggleTargetHUD != nullptr) m_pitemToggleTargetHUD->SetString(GetTargetHUDMenuString());
  }

  //Something of a misnomer since there are only two styles but this may change
  void CycleStyleHUD()
  {
    int style = (static_cast<int>(m_pnumberStyleHUD->GetValue()) + 1) % 2;
    m_pnumberStyleHUD->SetValue(static_cast<float>(style));

    SavePreference("SoftwareHUD", static_cast<DWORD>(style));

    if (m_pitemStyleHUD != nullptr) m_pitemStyleHUD->SetString(GetStyleHUDMenuString());
  }
  ...
  void ToggleLargeDeadZone(void)
  {
    ...
    if (m_pitemToggleLargeDeadZone != nullptr) m_pitemToggleLargeDeadZone->SetString(GetLargeDeadZoneMenuString());
  }
  ...
  void ToggleVirtualJoystick()
  {
    ...
    if (m_pitemToggleVirtualJoystick != nullptr) m_pitemToggleVirtualJoystick->SetString(GetVirtualJoystickMenuString());
  }
  ...
  void ToggleFlipY()
  {
    ...
    if (m_pitemToggleFlipY != nullptr) m_pitemToggleFlipY->SetString(GetFlipYMenuString());
  }
  ...
  void ToggleEnableFeedback()
  {
    ...
    if (m_pitemToggleEnableFeedback != nullptr) m_pitemToggleEnableFeedback->SetString(GetEnableFeedbackMenuString());
  }

  void RenderSizeChanged(bool bSmaller) override
  {
    if (bSmaller && GetFullscreen()) m_pwrapNumberStyleHUD->SetWrappedValue(new Number(1.0f));
    else m_pwrapNumberStyleHUD->SetWrappedValue(m_pnumberStyleHUD);
  }
  ...
  void SwitchSoundQuality()
  {
    ...
    ZSucceeded(m_pSoundEngine->SetQuality(m_soundquality));
    SavePreference("SoundQuality", static_cast<DWORD>(m_soundquality));

    if (m_pitemSoundQuality != nullptr) m_pitemSoundQuality->SetString(GetSoundQualityMenuString());
  };
  ...
  void ToggleSoundHardware()
  {
    ...
    SavePreference("SoundHardwareAcceleration", m_bEnableSoundHardware);

    ZSucceeded(m_pSoundEngine->EnableHardware(m_bEnableSoundHardware));

    if (m_pitemToggleSoundHardware != nullptr) m_pitemToggleSoundHardware->SetString(GetSoundHardwareMenuString());
  }
  ...
  void ToggleMusic()
  {
    ...
    SavePreference("Music", m_bMusic);

    UpdateMusic();

    if (m_pitemToggleMusic != nullptr) m_pitemToggleMusic->SetString(GetMusicMenuString());
  }
  ...
  void AdjustMusicVolume(float fDelta)
  {
    ...
    SavePreference("MusicGain", static_cast<DWORD>(-fNewValue));

    if (m_pitemMusicVolumeUp != nullptr) { m_pitemMusicVolumeUp->SetString(GetGainMenuString("Music", m_pnumMusicGain->GetValue(), c_fVolumeDelta)); }
    if (m_pitemMusicVolumeDown != nullptr) { m_pitemMusicVolumeDown->SetString(GetGainMenuString("Music", m_pnumMusicGain->GetValue(), -c_fVolumeDelta)); }
  ...
  void AdjustSFXVolume(float fDelta)
  {
    ...
    SavePreference("SFXGain", static_cast<DWORD>(-fNewValue));

    if (m_pitemSFXVolumeUp != nullptr) { m_pitemSFXVolumeUp->SetString(GetGainMenuString("Sound Effect", m_pnumSFXGain->GetValue(), c_fVolumeDelta)); }
    if (m_pitemSFXVolumeDown != nullptr) { m_pitemSFXVolumeDown->SetString(GetGainMenuString("Sound Effect", m_pnumSFXGain->GetValue(), -c_fVolumeDelta)); }
  }
  ...
  void AdjustVoiceOverVolume(float fDelta)
  {
    ...
    SavePreference("VoiceOverGain", static_cast<DWORD>(-fNewValue));

    if (m_pitemVoiceOverVolumeUp != nullptr) { m_pitemVoiceOverVolumeUp->SetString(GetGainMenuString("Voice Over", m_pnumVoiceOverGain->GetValue(), c_fVolumeDelta)); }
    if (m_pitemVoiceOverVolumeDown != nullptr) { m_pitemVoiceOverVolumeDown->SetString(GetGainMenuString("Voice Over", m_pnumVoiceOverGain->GetValue(), -c_fVolumeDelta)); }
  }

  ZString GetPostersMenuString() { return (m_pwrapImagePosters->GetImage() != Image::GetEmpty()) ? "Posters On " : "Posters Off "; }

  ZString GetDebrisMenuString() { return (m_pwrapGeoDebris->GetGeo() != Geo::GetEmpty()) ? "Debris On " : "Debris Off "; }

  ZString GetEnvironmentMenuString() { return (m_pwrapImageEnvironment->GetImage() == m_pimageEnvironment) ? "Environment On " : "Environment Off "; }

  ZString GetRoundRadarMenuString() { return (m_bRoundRadar) ? "Round Radar" : "Square Radar"; }

  ZString GetCensorChatsMenuString() { return (CensorDisplay()) ? "Censor Display" : "Don't Censor Display"; }

  ZString GetStickyChaseMenuString() { return m_bPreferChaseView ? "Default To Chase View" : "Default To Cockpit View"; }

  ZString GetFilterChatsToAllMenuString() { return trekClient.FilterChatsToAll() ? "Filter Chats Sent To All" : "Don't Filter Chats Sent To All"; }

  ZString GetFilterQuickCommsMenuString() { return trekClient.FilterQuickComms() ? "Filter Voice Commands" : "Don't Filter Voice Commands"; }

  ZString GetFilterLobbyChatsMenuString() { return trekClient.FilterLobbyChats() ? "Filter Chats Sent From Lobby" : "Don't Filter Chats Sent From Lobby"; }

  ZString GetLinearControlsMenuString() { return (m_bLinearControls) ? "Linear Control Response" : "Quadratic Control Response"; }

  ZString GetStarsMenuString() { return (m_pwrapImageStars->GetImage() != Image::GetEmpty()) ? "Stars On " : "Stars Off "; }

  ZString GetStrobesMenuString() { return ThingGeo::GetShowLights() ? "Strobes On " : "Strobes Off "; }

  ZString GetTrailsMenuString() { return ThingGeo::GetShowTrails() ? "Trails On " : "Trails Off "; }

  ZString GetBoundsMenuString() { return ThingGeo::GetShowBounds() ? "Bounds On " : "Bounds Off "; }

  ZString GetTransparentObjectsMenuString() { return ThingGeo::GetTransparentObjects() ? "TransparentObjects On " : "TransparentObjects Off "; }

  ZString GetLensFlareMenuString() { return (m_bLensFlare) ? "Lens Flare On " : "Lens Flare Off "; }
  ...
  ZString GetSoundHardwareMenuString() { return m_bEnableSoundHardware ? "Sound Card Acceleration On" : "Sound Card Acceleration Off"; }

  ZString GetMusicMenuString() { return (m_bMusic) ? "Music On " : "Music Off "; }

  ZString GetBidirectionalLightingMenuString() { return (m_bBidirectionalLighting) ? "Bidirectional Lighting On " : "Bidirectional Lighting Off "; }

  ZString GetChatHistoryHUDMenuString() { return (m_pboolChatHistoryHUD->GetValue()) ? "Chat History On " : "Chat History Off "; }

  ZString GetCenterHUDMenuString() { return (m_pboolCenterHUD->GetValue()) ? "Center HUD On " : "Center HUD Off "; }

  ZString GetTargetHUDMenuString() { return (m_pboolTargetHUD->GetValue()) ? "Target HUD On " : "Target HUD Off "; }
  ...
  ZString GetVirtualJoystickMenuString() { return (m_bEnableVirtualJoystick ? "Virtual Joystick On " : "Virtual Joystick Off "); }

  ZString GetFlipYMenuString() { return (m_bFlipY ? "Y Axis Flipped " : "Y Axis Not Flipped "); }

  ZString GetEnableFeedbackMenuString() { return (m_bEnableFeedback ? "Force Feedback Enabled " : "Force Feedback Disabled "); }
  ...
  ZString GetGainMenuString(const ZString &strSoundType, float fCurrentGain, float fDelta)
  {
    ...
    if (fCurrentGain >= 0 && fDelta > 0) strResult += "(maxed)";
    else if (fCurrentGain <= c_nMinGain && fDelta < 0) strResult += "(off)";
    else strResult += "to " + ZString(min(0, max(c_nMinGain, fCurrentGain + fDelta))) + " dB";

    return strResult;
  }

  void DoInputConfigure() override
  {
    CloseMenu();
    TRef<IPopup> ppopup = m_ptrekInput->CreateInputMapPopup(GetModeler(), TrekResources::SmallFont(), GetTime());
  ...
  void OnGameState() { GetPopupContainer()->OpenPopup(CreateMissionParametersPopup(GetModeler())); }

  void OnMenuCommand(IMenuItem *pitem) override
  {
    switch (pitem->GetID())
    {
      case idmConfigure:
  ...
  void SetJiggle(float jiggle) override { m_cameraControl.SetJiggle(jiggle); }

  void OverrideCamera(Time now, ImodelIGC *pmodelTarget, bool bOverridePosition) override
  {
    ...
    if (bOverridePosition)
    {
      ...
      if (pmodelTarget && (pmodelTarget->GetCluster() == pcluster))
      {
        ...
        if (pmodelTarget->GetObjectType() == OT_ship) SetTarget(pmodelTarget, c_cidNone);
      }
      else
      {
      ...
      trekClient.GetShip()->SetCluster(nullptr);
      trekClient.SetViewCluster(pcluster);
      TrackCamera(pshipSource->GetPosition(), *pPositionTarget, offset * 3.0f);
    }
    else m_positionOverrideTarget = pmodelTarget->GetPosition();
  ...
  {
    if (m_bUseOverridePosition) SetTarget(nullptr, c_cidNone);

    // end the overide camera's special status
    m_cm = cmExternal;

    if (trekClient.flyingF()) SetViewMode(vmCombat);
  ...
  void SetCameraMode(CameraMode cm) override
  {
    // Ignore the set display mode.
    if (m_cm == cmExternalOverride) return;
    ...
    if ((cm == cmCockpit) && m_bPreferChaseView) cm = cmExternalChase;
    ...
    if (m_cm != cm)
    {
      ...
      ThingSitePrivate *pSite;
      CastTo(pSite, trekClient.GetShip()->GetSourceShip()->GetThingSite());
      TRef<ThingGeo> pThing = pSite ? pSite->GetThingGeo() : nullptr;
      ...
      if (InternalCamera(cm))
      {
        ...
        if (pThing) pThing->SetShowDamage(false);
      }
      else if (ExternalCamera(cm)) { if (pThing) pThing->SetShowDamage(true); }

      UpdateOverlayFlags();

      // REVIEW: why not expose this in MDL?
      if (GetOverlayFlags() & (ofInCockpit | ofInChase)) m_pradarImage->SetRadarLOD(static_cast<RadarImage::RadarLOD>(m_radarCockpit));
      else if (GetOverlayFlags() & (ofInFlightCommand | ofInStationCommand)) m_pradarImage->SetRadarLOD(static_cast<RadarImage::RadarLOD>(m_radarCommand));
      else m_pradarImage->SetRadarLOD(RadarImage::c_rlNone);
    }
  }

  ViewMode GetViewMode(void) const override { return m_viewmode; }
  ...
  void SetViewMode(ViewMode vm, bool bForce = false) override
  {
    ZDebugOutput("SetViewMode(" + ZString(vm) + ")\n");
    ...
    if (bForce || vm != m_viewmode) AdjustCombatSize(vm);
    ...
    if (vm != m_viewmode)
    {
      ...
      switch (vm)
      {
        case vmHangar:
          ...
          {
            m_pimageBackdrop = new TranslateImage(CreatePaneImage(GetEngine(), SurfaceType2D(), false, m_pscreenBackdrop->GetPane()), Point(0, 0));
          }
        ...
        case vmLoadout:
          ...
          {
            m_pimageBackdrop = new TranslateImage(CreatePaneImage(GetEngine(), SurfaceType3D() | SurfaceTypeZBuffer(), false, m_pscreenBackdrop->GetPane()), Point(0, 0));
          }
        ...
        case vmCombat: assert(trekClient.GetViewCluster() == NULL);
          SetCameraMode(cmCockpit);
          m_pwrapImageBackdrop->SetImage(trekClient.GetCluster() ? m_pgroupImage3D : Image::GetEmpty());
          m_pscreenBackdrop = nullptr;
          m_pimageBackdrop = nullptr;
        ...
        case vmOverride: assert(false);// must call OverrideCamera()
          break;

        case vmUI: assert(false);// must call screen()
          break;
      ...
      if (trekClient.IsLockedDown()) StartLockDown(trekClient.GetLockDownReason());
    }
  }

  void EnableDisplacementCommandView(bool bEnable) override { m_bEnableDisplacementCommandView = bEnable; }

  OverlayMask GetOverlayFlags() const override { return m_voverlaymask[m_viewmode]; }

  void SetOverlayFlags(OverlayMask om) override
  {
    m_voverlaymask[m_viewmode] = om;
    if (m_pconsoleImage) m_pconsoleImage->SetOverlayFlags(om);

    if (om & ofTeam) m_pnumberTeamPaneCollapsed->SetValue(1.0f);
    else if (om & ofExpandedTeam) m_pnumberTeamPaneCollapsed->SetValue(0.0f);

    if ((om & ofInvestment) == 0) SetInvestAsteroid(0);
  ...
  void TurnOnOverlayFlags(OverlayMask om) override { SetOverlayFlags(m_voverlaymask[m_viewmode] | om); }

  void TurnOffOverlayFlags(OverlayMask om) override { SetOverlayFlags(m_voverlaymask[m_viewmode] & ~om); }

  void ToggleOverlayFlags(OverlayMask om) override { SetOverlayFlags(m_voverlaymask[m_viewmode] ^ om); }

  void ResetOverlayMask()
  {
    for (int i = 0; i < c_cViewModes; i++) m_voverlaymask[i] = 0;
  ...
  {
    if (bSet) TurnOnOverlayFlags(om);
    else TurnOffOverlayFlags(om);
  }

  void UpdateOverlayFlags()
  {
    bool bInCommand = GetViewMode() == vmCommand;
    bool bInFlight = trekClient.GetShip()->GetCluster() != nullptr;
    ...
    CopyOverlayFlags(ofInChase, bInFlight && ((GetCameraMode() == cmExternalChase) || ((GetCameraMode() == cmExternalMissile) && (m_pmissileLast == nullptr))));
  ...
  void TrackCamera(const Vector &position1, const Vector &position2, float length)
  {
    ...
    if (fabsf(dot_product) > 0.95f)
    {
      ...
      dot_product = up * axis;
      if (fabsf(dot_product) > 0.95f) up = orientation.GetRight();
  ...
  void ComputeExternalChase(IshipIGC *pShip, Orientation &orientationBfr, Vector &positionBfr, float &offset)
  {
    ...
    while ((m_turnRateSamples[iSampleIndex].time + fDelayTime) > m_turnRateSamples[m_turnRateSampleIndex].time) { iSampleIndex = ((iSampleIndex - 1) + ARRAY_OF_SAMPLES_SIZE) % ARRAY_OF_SAMPLES_SIZE; }
    ...
    positionBfr = shipPosition + (orientationBfr.GetUp() * fOldUpOffset) + (orientationBfr.GetBackward() * fOldForwardVelocityOffset) + (velocityVector * fCurrentVelocityOffset) + (currentLookOrientation.GetBackward() * fCurrentOrientationOffset) + (oldLookOrientation.GetBackward() * fOldOrientationOffset);
  ...
  void UpdateCamera(float dt)
  {
    if (trekClient.GetCluster())
    {
      ...
      else
      {
        ...
        if (CommandCamera(m_cm)) UpdateCommandView();
        else
        {
          ...
          switch (m_cm)
          {
              ...
              {
                {
                  ...
                  if (offset < fMin) offset = m_distanceExternalCamera = fMin;
                }
            ...
            case cmExternalFlyBy:
            {
              ...
              if (pmodelTarget != nullptr)
              {
                ...
                float dTarget;
                {
                  ...
                  float dSide = sqrt(delta.LengthSquared() + dBackward * dBackward);
            ...
            {
              if (!m_bUseOverridePosition) { TrackCamera(myPosition, m_positionOverrideTarget, pshipSource->GetRadius() * 3.0f); }
            ...
            case cmExternalStation:
            {
              ...
              if ((myTarget != nullptr) && (myTarget != pshipSource) && (pshipSource->GetCluster() == myTarget->GetCluster()) && (myTarget->GetHitTest() != nullptr) && pshipSource->CanSee(myTarget))
              {
                pPosition = &myPosition;
                pOrientation = &m_orientationExternalCamera;
          ...
          if (offset >= 0.0f)
          {
            m_cameraControl.SetPosition(offset == 0.0f ? *pPosition : (*pPosition + offset * pOrientation->GetBackward()));
      ...
      m_pCommandVisibleGeo->SetVisible(m_bCommandGrid || CommandCamera(m_cm) || (bAnimating && CommandCamera(m_cmOld)));

      {
        bool v = !(InternalCamera(m_cm) && ((!bAnimating) || InternalCamera(m_cmOld)));
  ...
  void AddMuzzleFlare(const Vector &vecDirection, float duration) override
  {
    /*  , doesn't work
  ...
  ZString GetFPSString(float fps, float mspf, Context *pcontext) override
  {
    ZString strPosition;

    if (g_pzfFrameDump && g_pzfFrameDump->IsValid())
    {
      ZString str = ZString(static_cast<int>(mspf)) + "," + ZString(static_cast<int>(fps)) + ",";

      int i;
      for (i = 0; i < 7; i++) str += ZString(g_fdInfo.objects_drawn[i]) + ",";

      for (i = 0; i < 7; i++) str += ZString(g_fdInfo.objects_insector[i]) + ",";
    ...
    if (trekClient.flyingF())
    {
      ...
      strPosition = " pos(" + ZString(static_cast<int>(myPosition.X())) + "," + ZString(static_cast<int>(myPosition.Y())) + "," + ZString(static_cast<int>(myPosition.Z())) + ")";
    }
    else strPosition = " pos: (Dead)";
    ...
    */
    ...
    if (trekClient.m_fm.IsConnected()) trekClient.m_fm.GetIPAddress(*trekClient.m_fm.GetServerConnection(), szRemoteAddress);
    else
      lstrcpy(szRemoteAddress, "N/A");

    return "TC:" + ZString(ThingGeo::GetTrashCount()) + " CC:" + ZString(ThingGeo::GetCrashCount()) + " Server: " + ZString(szRemoteAddress) + " fov: " + ((m_pcamera != nullptr) ? ZString(DegreesFromRadians(m_pcamera->GetFOV())) : ZString()) + strPosition + " lag: " + ZString(static_cast<long>(trekClient.m_serverLag)) + " ping: " + ZString(Time::Now() - trekClient.m_timeLastPing) + " sync: " + ZString(static_cast<int>(100.0f * trekClient.m_sync)) + " snds: " + ZString(nNumPlayingSoundBuffers);

    //return m_ptrekInput->GetFPSString();
  }

  void StartTraining(int iMissionIndex) override
  {
    if (Training::StartMission(iMissionIndex))
    {
      screen(ScreenIDCombat);

      if (m_cm == cmExternalOverride) EndOverrideCamera();
      ...
      // set the ship status
      ShipStatus status;
      status.SetHullID(trekClient.GetShip()->GetHullType()->GetObjectID());
  ...
  void PositionCommandView(const Vector *pposition, float dt) override
  {
    if (CommandCamera(m_cm))
    {
      ...
      else
      {
        ...
        if (m_distanceCommandCamera < fMin) m_distanceCommandCamera = fMin;
      }
    }
  }
    ...
    if (!m_pconsoleImage->DrawSelectionBox())
    {
      ...
      if (pmodelTarget != nullptr)
      {
        if (m_bTrackCommandView)
        {
          ...
          if ((pmodelTarget->GetCluster() == pcluster) && trekClient.GetShip()->CanSee(pmodelTarget))
          {
            ...
            m_positionCommandView = positionTarget + m_displacementCommandView;
          }
        }
      }
      /*
      ...
      */

      {
        ...
        if (l2 > 4.0f * s_fCommandViewDisplacementMax * s_fCommandViewDisplacementMax) m_positionCommandView *= 2.0f * s_fCommandViewDisplacementMax / sqrt(l2);
      }
    }
  ...
  void ChangeChatMessage(void) override {}
  ...
  */
  ...
  void OnPaint(HDC hdc, const WinRect &rect) override {}

  float GetDeltaTime(void) const override { return m_fDeltaTime; }

  void EvaluateFrame(Time time) override
  {
    ...
    if (bFirstFrame)
    {
      ...
      bFirstFrame = false;
      if (m_screen != ScreenIDCombat) return;
    }

    float dtime = m_fDeltaTime = time - m_timeLastFrame;
    ZAssert(dtime >= 0);
    ...
    {
      bool bEnable = m_bEnableVirtualJoystick && GetFullscreen() && GetPopupContainer()->IsEmpty() && trekClient.flyingF() && m_viewmode == vmCombat && ((m_voverlaymask[m_viewmode] & c_omBanishablePanes) == 0);
    ...
    if (m_pscreen) m_pscreen->OnFrame();
    if (m_pscreenBackdrop) m_pscreenBackdrop->OnFrame();
    CheckCountdownSound();

    // Handle AutoDownload as needed, if we're not downloading, do nothing
    if (trekClient.m_pAutoDownload)
    {
      if (GetWindow()->GetFullscreen()) trekClient.HandleAutoDownload(50);// give smaller time slice to allow for mouse to update
      else trekClient.HandleAutoDownload(500);// since the mouse is hardware in not full screen, the graphics engine doesn't need much CPU 
    }

    // receive network messages
    trekClient.m_lastUpdate = m_timeLastFrame;
    trekClient.m_now = time;
    if (FAILED(trekClient.ReceiveMessages())) return;//bug out
    ...
    trekClient.UpdateAmbientSounds(static_cast<DWORD>(dtime * 1000));
    UpdateMusic();
    ...
    if (m_pconsoleImage != nullptr && m_bOverlaysChanged)
    {
      UpdateOverlayFlags();
      m_bOverlaysChanged = false;
    }
    ...
    if (m_timeRejectQueuedCommand < time) RejectQueuedCommand(false);
    ...
    if (pcluster && m_screen == ScreenIDCombat)
    {
      ...
      double angleToPixels = static_cast<double>(rectImage.Size().X()) / static_cast<double>(m_cameraControl.GetFOV());
...
#else
      float fScreenXSize = rectImage.XMax() - rectImage.XMin(), fScreenYSize = rectImage.YMax() - rectImage.YMin(), fScreenSize = sqrtf((fScreenXSize * fScreenXSize) + (fScreenYSize * fScreenYSize)) * 0.5f;
#endif
      ...
      IsideIGC *psideMe;
      bool bAnyEnemyShips;
      if (pmodelTarget) bAnyEnemyShips = true;
      else
      {
      ...
      if (CommandCamera(m_cm))
      {
        ...
        float r = cosTheta2 > 0.0f ? static_cast<float>(c_fCommandGridRadius * sqrt(cosTheta2)) : 0.0f;
      ...
      for (ModelLinkIGC *l = models->first(); (l != nullptr); l = l->next())
      {
        ...
        if (pmodel->GetObjectType() == OT_ship)
        {
          ...
          if (pmodelAleph)
          {
            ...
            if (fRelativeVelocity > 0.0f) pship->GetThingSite()->SetTimeUntilAleph(fRange / fRelativeVelocity);
            else pship->GetThingSite()->SetTimeUntilAleph(-1.0f);
          }
        }

        // do all other stuff
        if (((pmodel != trekClient.GetShip()) && (pmodel != pshipParent)) || pmodel->GetVisibleF())
        {
          ObjectType type = pmodel->GetObjectType();
          if ((type != OT_ship) || (static_cast<IshipIGC *>(pmodel)->GetParentShip() == nullptr))
          {
            ThingSitePrivate *psite;
            CastTo(psite, pmodel->GetThingSite());
            ...
            //
            {
...

              {
                if (m_pcamera->TransformLocalToImage(positionModel, pointScreenPosition))
                {
                  ...
                  if (m_bRoundRadar)
                  {
                    ...
                    if (fDistanceSquared >= (fScreenSize * fScreenSize)) ucRadarState = c_ucRadarOffScreen;
                    else
                    {
                ...
                else ucRadarState = c_ucRadarOffScreen;
              }
              ...
              if (ucRadarState == c_ucRadarOffScreen)
              {
                ...
                if (m_bRoundRadar)
                {
                  ...
                  float fLength = sqrtf(vecDirection.X() * vecDirection.X() + vecDirection.Y() * vecDirection.Y());
                ...
                else
                {
                  ...
                  if (x == 0.0f) y = (y <= 0.0f) ? rectImage.YMin() : rectImage.YMax();
                  else if (y == 0.0f) x = (x <= 0.0f) ? rectImage.XMin() : rectImage.XMax();
                  else
                  {
              ...
              else
              {
                ...
                fScreenRadius = (sinAlpha < 1.0)
                                  ? static_cast<float>(angleToPixels * asin(sinAlpha))//hack ... 480/(50*pi/180) * 2asin(r/d)
                                  : 640.0f;
                if (fScreenRadius < CmdViewMaxIconScreenSize)
                {
                  if ((fScreenRadius < AnyViewMinRenderScreenSize) || CommandCamera(m_cm))
                  {
                ...
                if (!m_bRoundRadar)
                {
                  distanceToEdge = (pointScreenPosition.X() >= 0.0f) ? (rectImage.XMax() - pointScreenPosition.X()) : (pointScreenPosition.X() - rectImage.XMin());
                  {
                    float d = (pointScreenPosition.Y() >= 0.0f) ? (rectImage.YMax() - pointScreenPosition.Y()) : (pointScreenPosition.Y() - rectImage.YMin());
                    if (d < distanceToEdge) distanceToEdge = d;
                  }
                }
              ...
              psite->UpdateScreenPosition(pointScreenPosition, fScreenRadius, distanceToEdge, ucRadarState);
            }
            ...
            if (type == OT_ship)
            {
              ...
              if (stateM & miningMaskIGC)
              {
                ...
                if (pmodel && (pmodel->GetObjectType() == OT_asteroid))
                {
                  ...
                  static_cast<IasteroidIGC *>(pmodel)->MineOre(minedOre);
                }
              }
              ...
              if (visibleF && pthing)
              {
                auto afterburner = static_cast<IafterburnerIGC *>(pship->GetMountedPart(ET_Afterburner, 0));

                if (afterburner) { pthing->SetAfterburnerThrust(pship->GetOrientation().GetBackward(), afterburner->GetPower()); }
                else { pthing->SetAfterburnerThrust(Vector::GetZero(), 0.0f); }

                pthing->SetThrust(0.5f * (pship->GetControls().jsValues[3] + 1));

                if (stateM & miningMaskIGC) psite->ActivateBolt();
                else psite->DeactivateBolt();
              }
              else pthing->SetAfterburnerThrust(Vector::GetZero(), 0.0f);
            }
            else bSetVisible = true;
      ...
      if (CommandCamera(m_cm)) { m_cameraControl.SetZClip(zMin < 5.0f ? 5.0f : zMin, zMax); }
      else m_cameraControl.SetZClip(5.0f, 10000.0f);

      if ((pmodelTarget == nullptr) && bAnyEnemyShips && (!Training::IsTraining()))
      {
        //We have no target and there are enemy ships ... select an appropriate ship
        const Vector &position = pshipSource->GetPosition();

        ImodelIGC *m = FindTarget(pshipSource, (c_ttShip | c_ttEnemy | c_ttNearest), nullptr, pcluster, &position, nullptr, c_habmRescue);

        if (m == nullptr)
        {
          m = FindTarget(pshipSource, (c_ttShip | c_ttEnemy | c_ttNearest), nullptr, pcluster, &position, nullptr, c_habmMiner | c_habmBuilder);

          if (m == nullptr) { m = FindTarget(pshipSource, (c_ttShip | c_ttEnemy | c_ttNearest), nullptr, pcluster, &position); }
        }

        if (m) SetTarget(m, c_cidDefault);
      }

      m_pnumberFlash->SetValue(GetFlash(time) ? 1.0f : 0.0f);
    }

    if (m_pconsoleImage) m_pconsoleImage->Update(time);
    ...
    if (m_pgsc)
    {
      ...
      {
        //Countdown timer
        m_pgsc->SetTimeRemaining(static_cast<int>(pmp->GetCountDownTime() - dt));
      }
      else
      {
        //Countup timer
        m_pgsc->SetTimeElapsed(static_cast<int>(dt));
      }
    }
  }

  void OnEnterMission(void) override
  {
    if (!(Training::IsTraining() || (Training::GetTrainingMissionID() == Training::c_TM_7_Live)))
    {
      ...
      IstationIGC *pstation = trekClient.GetShip()->GetStation();
      if (pstation) trekClient.ReplaceLoadout(pstation);
    }
  }

  void PlayFFEffect(ForceEffectID effectID, LONG lDirection = 0) override { if (m_bEnableFeedback) m_ptrekInput->PlayFFEffect(effectID, lDirection); }

  void DoTrekUpdate(Time lastUpdate, Time now, float dt, bool activeControlsF)
  {
    if (trekClient.GetCluster() && GetWindow()->screen() == ScreenIDCombat)
    {
      ...
      if (GetFocus())
      {
        ...
        if (trekClient.flyingF())
        {
          ...
          if (!GetUI() && activeControlsF)
          {
            ...
            if (m_ptrekInput->IsTrekKeyDown(TK_ThrustLeft, bAllowKeyboardMovement)) buttonsM |= leftButtonIGC;
            if (m_ptrekInput->IsTrekKeyDown(TK_ThrustRight, bAllowKeyboardMovement)) buttonsM |= rightButtonIGC;
            if (m_ptrekInput->IsTrekKeyDown(TK_ThrustUp, bAllowKeyboardMovement)) buttonsM |= upButtonIGC;
            if (m_ptrekInput->IsTrekKeyDown(TK_ThrustDown, bAllowKeyboardMovement)) buttonsM |= downButtonIGC;
            if (m_ptrekInput->IsTrekKeyDown(TK_ThrustForward, bAllowKeyboardMovement)) buttonsM |= forwardButtonIGC;
            if (m_ptrekInput->IsTrekKeyDown(TK_ThrustBackward, bAllowKeyboardMovement)) buttonsM |= backwardButtonIGC;
            if (m_ptrekInput->IsTrekKeyDown(TK_FireBooster, bAllowKeyboardMovement)) buttonsM |= afterburnerButtonIGC;

            if (fAutoPilot)
            {
              if (oldButtonsM != buttonsM) bControlsInUse = true;
              else { bControlsInUse = bControlsInUse || js.button1 || js.button2 || js.button3 || js.button4 || js.button5 || js.button6; }
              ...
              else
              {
                ...
                bControlsInUse = bControlsInUse || (js.controls.jsValues[c_axisYaw] - trekClient.trekJoyStick[c_axisYaw] < -c_fAutopilotDisengage) || (js.controls.jsValues[c_axisYaw] - trekClient.trekJoyStick[c_axisYaw] > c_fAutopilotDisengage) || (js.controls.jsValues[c_axisPitch] - trekClient.trekJoyStick[c_axisPitch] < -c_fAutopilotDisengage) || (js.controls.jsValues[c_axisPitch] - trekClient.trekJoyStick[c_axisPitch] > c_fAutopilotDisengage) || (js.controls.jsValues[c_axisRoll] - trekClient.trekJoyStick[c_axisRoll] < -c_fAutopilotDisengage) || (js.controls.jsValues[c_axisRoll] - trekClient.trekJoyStick[c_axisRoll] > c_fAutopilotDisengage);
              }
          ...
          IshipIGC *pshipParent = trekClient.GetShip()->GetParentShip();
          if (fAutoPilot && (pshipParent == nullptr))
          {
          ...
          else// autoPilot doesn't need these
          {
            ...
            if (js.button1 & buttonDown) { buttonsM |= trekClient.fGroupFire ? allWeaponsIGC : oneWeaponIGC; }

            if (js.button2 & buttonDown) buttonsM |= missileFireIGC;

            if (js.button3 & buttonDown) buttonsM |= afterburnerButtonIGC;
          }

          if (m_ptrekInput->IsTrekKeyDown(TK_DropMine, bAllowKeyboardMovement)) buttonsM |= mineFireIGC;
          else buttonsM &= ~mineFireIGC;

          if (m_ptrekInput->IsTrekKeyDown(TK_DropChaff, bAllowKeyboardMovement)) buttonsM |= chaffFireIGC;
          else buttonsM &= ~chaffFireIGC;
          ...
          if (m_cm == cmCockpit)
          {
            ...
            {
              float fov = (s_fMinFOV + 0.5f * (s_fMaxFOV - s_fMinFOV)) + (0.5f * (s_fMaxFOV - s_fMinFOV)) * js.controls.jsValues[c_axisThrottle];
            ...
            else
            {
              ...
              {
                fov -= dt;
                if (fov < s_fMinFOV) fov = s_fMinFOV;
              ...
              {
                fov += dt;
                if (fov > s_fMaxFOV) fov = s_fMaxFOV;

                m_cameraControl.SetFOV(fov);
              }

              if (trekClient.GetShip()->GetBaseHullType() != nullptr)
              {
                ...
                const IhullTypeIGC *pht = trekClient.GetShip()->GetHullType();
                {
                  ...
                  if (pitch > maxSlewRate) js.controls.jsValues[c_axisPitch] *= maxSlewRate / pitch;
                }
                {
                  float yaw = pht->GetMaxTurnRate(c_axisYaw);
                  if (yaw > maxSlewRate) js.controls.jsValues[c_axisYaw] *= maxSlewRate / yaw;
                }
              }
            }
          }
          ...
          trekClient.GetShip()->SetStateBits(buttonsMaskIGC | weaponsMaskIGC | selectedWeaponMaskIGC | missileFireIGC | mineFireIGC | chaffFireIGC, buttonsM);

          if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(js.hat);
          else m_cameraControl.SetHeadOrientation(0.0f);

          m_pplayerThrottle->SetValue(pshipParent ? 0.0f : ((buttonsM & (leftButtonIGC | rightButtonIGC | upButtonIGC | downButtonIGC | forwardButtonIGC | backwardButtonIGC | afterburnerButtonIGC)) ? 1.0f : (js.controls.jsValues[c_axisThrottle] * 0.5f + 0.5f)));
        }
        ...
        if (CommandCamera(m_cm) && m_bEnableDisplacementCommandView && !m_pconsoleImage->DrawSelectionBox())
        {
          ...
          if (bInside)
          {
            ...
            static constexpr float maxGutter = 12.0f;

            if (point.X() < gutter) dRight = delta * (point.X() - gutter) / maxGutter;
            else
            {
              float x = rectImage.XSize() - point.X();
              if (x < gutter) dRight = delta * (gutter - x) / maxGutter;
            }

            if (point.Y() < gutter) dUp = delta * (point.Y() - gutter) / maxGutter;
            else
            {
              float y = rectImage.YSize() - point.Y();
              if (y < gutter) dUp = delta * (gutter - y) / maxGutter;
            }
          }

          // Cheat ... horizontal is always parallel to the plane

          if (m_ptrekInput->IsTrekKeyDown(TK_YawLeft, bAllowKeyboardMovement)) dRight -= delta;
          else if (m_ptrekInput->IsTrekKeyDown(TK_YawRight, bAllowKeyboardMovement)) dRight += delta;

          if (m_ptrekInput->IsTrekKeyDown(TK_PitchUp, bAllowKeyboardMovement)) dUp -= delta;
          else if (m_ptrekInput->IsTrekKeyDown(TK_PitchDown, bAllowKeyboardMovement)) dUp += delta;

          if (m_ptrekInput->IsTrekKeyDown(TK_ZoomOut, bAllowKeyboardMovement))
          {
            m_distanceCommandCamera += delta * 2.0f;
            if (m_distanceCommandCamera > s_fCommandViewDistanceMax) m_distanceCommandCamera = s_fCommandViewDistanceMax;
          }
          else if (m_ptrekInput->IsTrekKeyDown(TK_ZoomIn, bAllowKeyboardMovement))
          {
            m_distanceCommandCamera -= delta * 2.0f;

            if (m_distanceCommandCamera < s_fCommandViewDistanceMin) m_distanceCommandCamera = s_fCommandViewDistanceMin;
          }
          ...
          if (m_ptrekInput->IsTrekKeyDown(TK_RollRight, bAllowKeyboardMovement))
          {
            ...
            if (m_rollCommandCamera >= 2.0f * pi) m_rollCommandCamera -= 2.0f * pi;
          ...
          else if (m_ptrekInput->IsTrekKeyDown(TK_RollLeft, bAllowKeyboardMovement))
          {
            ...
            if (m_distanceCommandCamera < 0.0f) m_distanceCommandCamera += 2.0f * pi;
          ...
          if (dUp)
          {
            ...
            float l = sqrt(up.x * up.x + up.y * up.y);
            assert(l != 0.0f);
        ...
        {
          if (m_ptrekInput->IsTrekKeyDown(TK_PitchUp, bAllowKeyboardMovement)) m_orientationExternalCamera.Pitch(dt);
          else if (m_ptrekInput->IsTrekKeyDown(TK_PitchDown, bAllowKeyboardMovement)) m_orientationExternalCamera.Pitch(-dt);

          if (m_ptrekInput->IsTrekKeyDown(TK_YawLeft, bAllowKeyboardMovement)) m_orientationExternalCamera.Yaw(dt);
          else if (m_ptrekInput->IsTrekKeyDown(TK_YawRight, bAllowKeyboardMovement)) m_orientationExternalCamera.Yaw(-dt);

          if (m_ptrekInput->IsTrekKeyDown(TK_RollLeft, bAllowKeyboardMovement)) m_orientationExternalCamera.Roll(dt);
          else if (m_ptrekInput->IsTrekKeyDown(TK_RollRight, bAllowKeyboardMovement)) m_orientationExternalCamera.Roll(-dt);

          if (m_ptrekInput->IsTrekKeyDown(TK_ZoomOut, bAllowKeyboardMovement))
          {
            m_distanceExternalCamera += dt * m_distanceExternalCamera;
            if (m_distanceExternalCamera > s_fExternalViewDistanceMax) m_distanceExternalCamera = s_fExternalViewDistanceMax;
          }
          else if (m_ptrekInput->IsTrekKeyDown(TK_ZoomIn, bAllowKeyboardMovement))
          {
            m_distanceExternalCamera -= dt * m_distanceExternalCamera;

            if (m_distanceExternalCamera < s_fExternalViewDistanceMin) m_distanceExternalCamera = s_fExternalViewDistanceMin;
          }
        }
      ...
      //Change sounds for warning state as appropriate
      {
        ...
        {
          if ((wm & c_wmOutOfBounds) && ((trekClient.wmOld & c_wmOutOfBounds) == 0)) trekClient.PlaySoundEffect(salBoundryExceededSound);

          if ((wm & c_wmCrowdedSector) && ((trekClient.wmOld & c_wmCrowdedSector) == 0)) trekClient.PlaySoundEffect(salSectorCrowdedSound);
      ...
      constexpr float alphaless = 0.55f;

      auto colorHUDshadows = Color(45, 36, 59);
    ...
    if (trekClient.m_fm.IsConnected()) trekClient.SendMessages();
  ...
  bool OnKey(IInputProvider *pprovider, const KeyState &ks, bool &bForceTranslate) override
  {
    if (IsInputEnabled(ks.vk))
    {
      ...
      if (TK_NoKeyMapping != tk)
      {
        if (ks.bDown)
        {
          ...
          if (Training::RecordKeyPress(tk))
          {
            if (trekClient.IsInGame() && ((GetViewMode() != vmOverride) || (tk == TK_StartChat)) && !trekClient.IsLockedDown()) OnTrekKey(tk);
  ...
  {
    if (!GetUI()) return m_pconsoleImage->OnChar(ks);
    return false;
  }

  void SetTargetGeo(ImodelIGC *pmodel) override
  {
    ...
    if (pmodel != nullptr)
    {
      if (pmodel->GetObjectType() == OT_ship) pmodel = static_cast<IshipIGC *>(pmodel)->GetSourceShip();

      if ((pmodel != trekClient.GetShip()->GetSourceShip()) && (pmodel->GetHitTest() != nullptr) && (pmodel->GetCluster() == trekClient.GetCluster()) && trekClient.GetShip()->CanSee(pmodel))
      {
        ...
        if (pthingSite)
        {
          ...
          if (pthing) m_cameraControl.m_pgeoTarget = pthing->GetTargetGeo();
        }
      }
  ...
  void SetTarget(ImodelIGC *pmodel, CommandID cid) override
  {
    if (pmodel && trekClient.GetShip()->GetCommandTarget(c_cmdCurrent) != pmodel) StartSound(newTargetSound);

    if (cid == c_cidDefault) cid = pmodel ? trekClient.GetShip()->GetDefaultOrder(pmodel) : c_cidNone;
    ...
    if (trekClient.m_fm.IsConnected())
    {
      ...
      BEGIN_PFM_CREATE(trekClient.m_fm, pfmOC, CS, ORDER_CHANGE) END_PFM_CREATE
    ...
    if (CommandCamera(m_cm))
    {
      if (m_cameraControl.GetAnimation() < 1.0f) m_cameraControl.SetAnimation(1.0f);

      if (pmodel && !m_bTrackCommandView)
      {
        if (pmodel->GetObjectType() == OT_ship) pmodel = static_cast<IshipIGC *>(pmodel)->GetSourceShip();

        if ((pmodel->GetCluster() == trekClient.GetCluster()) && trekClient.GetShip()->CanSee(pmodel)) m_positionCommandView = pmodel->GetPosition();
      }
  ...
  void SetAccepted(ImodelIGC *pmodel, CommandID cid) override
  {
  ...
  void SetTimeDamaged(Time now) override
  {
    if (now <= m_timeLastDamage + c_dtFlashingDuration)
    {
      ...
      float n = floor(dt / (c_dtFlashOn + c_dtFlashOff));

      m_timeLastDamage += n * (c_dtFlashOn + c_dtFlashOff);
    }
    else m_timeLastDamage = now;//Start flashing now
  }
  ...
  void TrekFindTarget(int ttMask, TrekKey tk, TrekKey tkNearest, TrekKey tkPrevious, AbilityBitMask abm = 0)
  {
    assert((ttMask & c_ttFront) == 0);

    if (tk == tkNearest) ttMask |= c_ttNearest;
    else if (tk == tkPrevious) ttMask |= c_ttPrevious;

    const Vector *pposition;
    if (trekClient.GetViewCluster())
    {
      IstationIGC *pstation = trekClient.GetShip()->GetStation();
      if (pstation && (pstation->GetCluster() == trekClient.GetViewCluster())) pposition = &(pstation->GetPosition());
      else pposition = &(Vector::GetZero());
    }
    else pposition = &(trekClient.GetShip()->GetPosition());

    ImodelIGC *m = FindTarget(trekClient.GetShip()->GetSourceShip(), ttMask, trekClient.GetShip()->GetCommandTarget(c_cmdCurrent), trekClient.GetCluster(), pposition, nullptr, abm);

    SetTarget(m, c_cidDefault);
  }

  void ToggleWeapon(Mount mount) override
  {
    IshipIGC *pship = trekClient.GetShip();

    if (pship->GetParentShip() == nullptr)
    {
      Mount nHardpoints = pship->GetHullType()->GetMaxFixedWeapons();
      auto pweapon = static_cast<IweaponIGC *>(pship->GetMountedPart(ET_Weapon, mount));
      ...
      if (pweapon && mount < nHardpoints)
      {
        ...
        if (trekClient.fGroupFire || pweapon->GetMountID() != mountSelected)
        {
  ...
  void SwapPart(EquipmentType et, Mount mount) override
  {
    IshipIGC *pship = trekClient.GetShip();

    if ((pship->GetParentShip() == nullptr) && trekClient.flyingF())
    {
      PartMask maxPartMask = pship->GetHullType()->GetPartMask(et, mount);
      IpartIGC *ppartOld = pship->GetMountedPart(et, mount);

      if (trekClient.SelectCargoPartOfType(et, maxPartMask, ppartOld ? ppartOld->GetPartType() : nullptr))
      {
        ...
        {
          if (ppartOld) trekClient.SwapPart(ppartOld, mountNew);
          else if (ppartNew) trekClient.SwapPart(ppartNew, mount);

          if (ppartNew) trekClient.PlaySoundEffect(mountSound, trekClient.GetShip());
          else if (ppartOld) trekClient.PlaySoundEffect(unmountSound, trekClient.GetShip());

          // set the inventory slot to the next item of that type
          trekClient.NextCargoPart();
          trekClient.SelectCargoPartOfType(et, maxPartMask, ppartNew ? ppartNew->GetPartType() : nullptr);

          trekClient.GetCore()->GetIgcSite()->LoadoutChangeEvent(pship, trekClient.GetCargoPart(), c_lcCargoSelectionChanged);
        }
      }
      else trekClient.PlaySoundEffect(salCargoFullSound, trekClient.GetShip());
    }
  }
  ...
  void SwapWeapon(Mount mount)
  {
    ...
    if ((pship->GetParentShip() == nullptr) && trekClient.flyingF())
    {
      ...
      if (mount < nHardpoints) SwapPart(ET_Weapon, mount);
    }
  }
  ...
  void SwapTurret(int id)
  {
    ...
    if ((pship->GetParentShip() == nullptr) && trekClient.flyingF())
    {
      ...
      if (id < nTurrets) SwapPart(ET_Weapon, id + nFixedHardpoints);
    }
  }
  ...
  float GetHUDStyle(void) const override { return m_pnumberStyleHUD->GetValue(); }

  void SetHUDStyle(float newStyle) override { m_pnumberStyleHUD->SetValue(newStyle); }
  ...
  class HelpPosition : public PointValue, public IEventSink
  {
    bool m_bOn;
    float m_timeStart;
    float m_valueStart;
  ...
  public:
    HelpPosition(Number *ptime, IEventSource *peventSource)
      : PointValue(ptime),
        ...
        m_positionOn(10, 10) { peventSource->AddSink(this); }
    ...
    {
      if (m_bOn) m_value = min(1, m_valueStart + (GetTime() - m_timeStart));
      else m_value = max(0, m_valueStart - (GetTime() - m_timeStart));
  ...
  {
  public:
    TRef<ZFile> Include(const ZString &str) override
    {
      HKEY hKey;

      if (ERROR_SUCCESS != ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, "", REG_OPTION_NON_VOLATILE, KEY_READ, nullptr, &hKey, nullptr)) return nullptr;

      char buf[128];
      DWORD dwSize = sizeof(buf);
      DWORD dwType = REG_SZ;

      ::RegQueryValueEx(hKey, str, nullptr, &dwType, (BYTE *) buf, &dwSize);
      RegCloseKey(hKey);
  ...
  ZString GetProductID()
  {
    ...
    if (ERROR_SUCCESS == ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, ALLEGIANCE_REGISTRY_KEY_ROOT, 0, "", REG_OPTION_NON_VOLATILE, KEY_READ, nullptr, &hKey, nullptr))
    {
      ...
      ::RegQueryValueEx(hKey, "PID", nullptr, &dwType, (BYTE *) buf, &dwSize);
      RegCloseKey(hKey);

      if (dwType == REG_SZ) return ZString(buf);
    }
  ...
  void InitializeHelp()
  {
    ...
    m_phelp->SetString("pid", GetProductID());
    {
      ...
      if (vi.Load()) m_phelp->SetString("ver", vi.GetProductVersionString());
      else m_phelp->SetString("ver", ZString("unknown"));
    }

    m_phelpPosition = new HelpPosition(GetTime(), m_phelp->GetEventSourceClose());

    m_pwrapImageHelp->SetImage(new TransformImage(CreatePaneImage(GetEngine(), SurfaceType2D() | SurfaceType3D(), true, m_phelp), new TranslateTransform2(m_phelpPosition)));
  }

  void OnHelp(bool bOn) override
  {
  ...
  bool OnTrekKeyFilter(TrekKey tk)
  {
    if (m_screen != ScreenIDSplashScreen)
    {
      switch (tk)
      {
        ...
        case TK_MainMenu:
          if ((!trekClient.IsInGame() || (GetViewMode() != vmOverride)) && !trekClient.IsLockedDown() && (m_pconsoleImage == nullptr || !m_pconsoleImage->IsComposing()))
          {
            ShowMainMenu();
            return true;
          }
        ...
        case TK_ChatPageUp:
          if (m_pchatListPane != nullptr) m_pchatListPane->PageUp();
          return true;

        case TK_ChatPageDown:
          if (m_pchatListPane != nullptr) m_pchatListPane->PageDown();
          return true;

        case TK_QuickChatMenu:
          if (trekClient.MyMission() && (!trekClient.IsInGame() || (GetViewMode() != vmOverride)) && !trekClient.IsLockedDown() && (m_screen == ScreenIDGameOverScreen || m_screen == ScreenIDTeamScreen || m_screen == ScreenIDCombat)) OpenMainQuickChatMenu();
          return true;
      }
    }
  ...
  void OnTrekKey(TrekKey tk) override
  {
    bool suicideF = false;
    ...
    switch (tk)
    {
      ...
      */
      ...
      case TK_ToggleMouse: { m_bEnableVirtualJoystick = !m_bEnableVirtualJoystick; }
      break;
      ...
      {
        if (m_pwrapImageLOD->GetImage() == Image::GetEmpty()) m_pwrapImageLOD->SetImage(m_pimageLOD);
        else m_pwrapImageLOD->SetImage(Image::GetEmpty());
      }
      break;
      ...
      case TK_ToggleCloak: { if (trekClient.GetShip()->GetParentShip() == nullptr) trekClient.GetShip()->SetStateBits(cloakActiveIGC, Training::ApproveActions(trekClient.GetShip()->GetStateM() ^ cloakActiveIGC)); }
      break;
      ...
      case TK_Ripcord:
      {
        if (!Training::IsTraining())
        {
          if ((trekClient.GetShip()->GetParentShip() == nullptr) && trekClient.GetShip()->GetCluster())
          {
            if ((trekClient.GetShip()->GetFlag() != NA) || trekClient.GetShip()->GetBaseHullType()->HasCapability(c_habmNoRipcord)) trekClient.PlayNotificationSound(salShipCantRipcordSound, trekClient.GetShip());
            else
            {
              IclusterIGC *pcluster = nullptr;
              if (!trekClient.GetShip()->fRipcordActive())
              {
                ImodelIGC *pmodel = trekClient.GetShip()->GetCommandTarget(c_cmdCurrent);

                if (!pmodel) pmodel = trekClient.GetShip()->GetCommandTarget(c_cmdAccepted);

                if (pmodel) pcluster = trekClient.GetCluster(trekClient.GetShip(), pmodel);

                if (!pcluster) pcluster = trekClient.GetShip()->GetCluster();
                assert(pcluster);
              }
      ...
      case TK_PrevWeapon: { if (trekClient.GetShip()->GetParentShip() == nullptr) trekClient.PreviousWeapon(); }
      break;

      case TK_NextWeapon: { if (trekClient.GetShip()->GetParentShip() == nullptr) trekClient.NextWeapon(); }
      break;

      case TK_ToggleGroupFire:
      {
        if (trekClient.GetShip()->GetParentShip() == nullptr) trekClient.fGroupFire = !trekClient.fGroupFire;

        if (trekClient.fGroupFire) trekClient.PlaySoundEffect(groupWeaponsSound, trekClient.GetShip());
        else trekClient.PlaySoundEffect(ungroupWeaponsSound, trekClient.GetShip());
      }
      break;

      case TK_SwapWeapon1: { SwapWeapon(0); }
      break;

      case TK_SwapWeapon2: { SwapWeapon(1); }
      break;

      case TK_SwapWeapon3: { SwapWeapon(2); }
      break;

      case TK_SwapWeapon4: { SwapWeapon(3); }
      break;

      case TK_SwapTurret1: { SwapTurret(0); }
      break;

      case TK_SwapTurret2: { SwapTurret(1); }
      break;

      case TK_SwapTurret3: { SwapTurret(2); }
      break;

      case TK_SwapTurret4: { SwapTurret(3); }
      break;
      ...
      case TK_PromoteTurret4:
      {
        ...
        if (pht)
        {
          Mount turret = static_cast<Mount>(tk - TK_PromoteTurret1) + pht->GetMaxFixedWeapons();
          if (turret < pht->GetMaxWeapons())
          {
            trekClient.SetMessageType(BaseClient::c_mtGuaranteed);
            BEGIN_PFM_CREATE(trekClient.m_fm, pfmPromote, C, PROMOTE) END_PFM_CREATE;
      ...
      case TK_SwapMissile: { SwapPart(ET_Magazine, 0); }
      break;

      case TK_SwapChaff: { SwapPart(ET_ChaffLauncher, 0); }
      break;

      case TK_SwapShield: { SwapPart(ET_Shield, 0); }
      break;

      case TK_SwapCloak: { SwapPart(ET_Cloak, 0); }
      break;

      case TK_SwapAfterburner: { SwapPart(ET_Afterburner, 0); }
      break;

      case TK_SwapMine: { SwapPart(ET_Dispenser, 0); }
      break;
      ...
      case TK_ToggleWeapon1: { ToggleWeapon(0); }
      break;

      case TK_ToggleWeapon2: { ToggleWeapon(1); }
      break;

      case TK_ToggleWeapon3: { ToggleWeapon(2); }
      break;

      case TK_ToggleWeapon4: { ToggleWeapon(3); }
      break;
      ...
      case TK_Reload:
      {
        if (trekClient.GetShip()->GetParentShip() == nullptr)
        {
          ...
          if ((stateM & (oneWeaponIGC | allWeaponsIGC)) == 0) trekClient.Reload(trekClient.GetShip(), nullptr, ET_Weapon);

          if ((stateM & afterburnerButtonIGC) == 0) trekClient.Reload(trekClient.GetShip(), nullptr, ET_Afterburner);

          if ((stateM & missileFireIGC) == 0) trekClient.Reload(trekClient.GetShip(), static_cast<IlauncherIGC *>(trekClient.GetShip()->GetMountedPart(ET_Magazine, 0)), ET_Magazine);
          if ((stateM & mineFireIGC) == 0) trekClient.Reload(trekClient.GetShip(), static_cast<IlauncherIGC *>(trekClient.GetShip()->GetMountedPart(ET_Dispenser, 0)), ET_Dispenser);
          if ((stateM & chaffFireIGC) == 0) trekClient.Reload(trekClient.GetShip(), static_cast<IlauncherIGC *>(trekClient.GetShip()->GetMountedPart(ET_ChaffLauncher, 0)), ET_ChaffLauncher);
      ...
      case TK_DropCargo:
      {
        if (trekClient.flyingF() && (trekClient.GetShip()->GetParentShip() == nullptr))
        {
          ...
          if (ppart)
          {
            ...
            for (Mount mount = -c_maxCargo; mount < 0; ++mount)
            {
              ...
              IpartTypeIGC *ppartTypePrev = ppartPrev ? ppartPrev->GetPartType() : nullptr;
          ...
          else trekClient.NextCargoPart();
          trekClient.GetCore()->GetIgcSite()->LoadoutChangeEvent(trekClient.GetShip(), trekClient.GetCargoPart(), c_lcCargoSelectionChanged);
        }
      }
      break;

      case TK_ViewRearLeft: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi * 1.25f); }
      break;

      case TK_ViewRearRight: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi * 0.75f); }
      break;

      case TK_ViewFrontLeft: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi * 1.75f); }
      break;

      case TK_ViewFrontRight: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi * 0.25f); }
      break;

      case TK_ViewRear: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi); }
      break;

      case TK_ViewLeft: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi * 1.5f); }
      break;

      case TK_ViewRight: { if ((m_cm == cmCockpit) || (m_cm == cmExternalChase)) m_cameraControl.SetHeadOrientation(pi * 0.5f); }
      break;

      case TK_ViewExternal: { SetViewToCombatMode((GetCameraMode() != cmExternal) ? cmExternal : cmCockpit); }
      break;

      case TK_ViewExternalOrbit: { SetViewToCombatMode((GetCameraMode() != cmExternalOrbit) ? cmExternalOrbit : cmCockpit); }
      break;

      case TK_ViewExternalStation: { SetViewToCombatMode((GetCameraMode() != cmExternalStation) ? cmExternalStation : cmCockpit); }
      break;

      case TK_ViewFlyby: { SetViewToCombatMode((GetCameraMode() != cmExternalFlyBy) ? cmExternalFlyBy : cmCockpit); }
      break;

      case TK_ViewMissile: { SetViewToCombatMode((GetCameraMode() != cmExternalMissile) ? cmExternalMissile : cmCockpit); }
      break;
      ...
      case TK_ViewTarget:
      {
        if (trekClient.flyingF())
        {
          if (GetCameraMode() != cmExternalTarget) SetCameraMode(cmExternalTarget);
          else SetCameraMode(cmCockpit);
        }
      }
      break;
      ...
      */
      ...
      case TK_ViewBase: { SetViewToCombatMode((GetCameraMode() != cmExternalBase) ? cmExternalBase : cmCockpit); }
      break;
      ...
      case TK_ConModeNav:
      case TK_ViewSector: { if (GetViewMode() != vmOverride) ToggleOverlayFlags(ofSectorPane); }
      break;
      ...
      case TK_ConModeCommand:
      {
        if (CommandCamera(m_cm))
        {
          if (GetOverlayFlags() & c_omBanishablePanes) TurnOffOverlayFlags(c_omBanishablePanes);
          else if (!trekClient.GetShip()->IsGhost())
          {
            if (trekClient.flyingF()) SetViewMode(vmCombat);
            else SetViewMode(vmHangar);
          }
        }
        else
        {
          if ((trekClient.GetViewCluster() == nullptr) && !trekClient.flyingF())
          {
            IstationIGC *pstation = trekClient.GetShip()->GetStation();
            assert(pstation);
          ...
          {
            SetViewMode(vmCommand);
            PositionCommandView(nullptr, 2.0f);
          }
        }
      }
      break;
      ...
      {
        if (m_cmPreviousCommand == cmExternalCommandViewTD) m_cmPreviousCommand = cmExternalCommandView34;
        else if (m_cmPreviousCommand == cmExternalCommandView34) m_cmPreviousCommand = cmExternalCommandViewTD;
      ...
      case TK_CycleRadar:
      {
        ...
        {
          m_radarCockpit = (m_radarCockpit + 1) % (static_cast<int>(RadarImage::c_rlMax) + 1);
          m_pradarImage->SetRadarLOD(static_cast<RadarImage::RadarLOD>(m_radarCockpit));

          trekClient.PostText(false, "%s", c_szRadarLODs[m_radarCockpit]);
        }
        else if (GetViewMode() == vmCommand)
        {
          m_radarCommand = (m_radarCommand + 1) % (static_cast<int>(RadarImage::c_rlMax) + 1);
          m_pradarImage->SetRadarLOD(static_cast<RadarImage::RadarLOD>(m_radarCommand));
      ...
      case TK_ConModeCombat:
      {
        ...
        else SetViewToCombatMode(cmCockpit);
      }
      break;
      ...
      case TK_ConModeInventory:
      {
        ...
        else if (trekClient.GetShip()->IsGhost()) TurnOffOverlayFlags(c_omBanishablePanes);
        else if (trekClient.GetShip()->GetStation() != nullptr)
        {
          if (GetViewMode() != vmLoadout) SetViewMode(vmLoadout);
          else if (GetOverlayFlags() & c_omBanishablePanes) TurnOffOverlayFlags(c_omBanishablePanes);
          else SetViewMode(vmHangar);
        }
        else ToggleOverlayFlags(ofInventory);
      }
      break;

      case TK_ConModeInvest:
      {
        if (GetViewMode() != vmOverride && trekClient.m_fm.IsConnected() && trekClient.MyMission()->AllowDevelopments())
        {
      ...
      case TK_ConModeTeam:
      {
        ...
        if (GetViewMode() != vmOverride)
        {
          ...
          else if (GetOverlayFlags() & ofExpandedTeam) TurnOffOverlayFlags(ofExpandedTeam);
          else TurnOnOverlayFlags(ofExpandedTeam);
        }
      }
      break;
      ...
      case TK_ConModeTeleport:
      {
        ...
        if (trekClient.GetShip()->GetStation() && !trekClient.GetShip()->IsGhost())
        {
          ...
          if (GetOverlayFlags() & ofTeleportPane) TurnOffOverlayFlags(ofTeleportPane);
          else TurnOnOverlayFlags(ofTeleportPane);
        }
      }
      break;
      ...
      case TK_TargetHostile: // target the thing that has done the most damage to me recently
      {
        ...
        else TrekFindTarget((c_ttShip | c_ttEnemy | c_ttNearest), tk, TK_TargetEnemyNearest, TK_TargetEnemyPrev, c_habmRescue | c_habmMiner | c_habmBuilder);
      }
      break;
      ...
      case TK_TargetCenter: // target the ship closest to the reticle
      {
        if (trekClient.flyingF())
        {
          ...
          {
            Mount turretID = trekClient.GetShip()->GetTurretID();
            if (turretID == NA) porientation = &(pship->GetOrientation());
          }
          else pship = trekClient.GetShip();

          ImodelIGC *m = FindTarget(pship, (c_ttFront | c_ttAllTypes | c_ttAllSides), trekClient.GetShip()->GetCommandTarget(c_cmdCurrent), nullptr, nullptr, porientation);
      ...
      case TK_TargetEnemyPrev: { TrekFindTarget((c_ttShip | c_ttEnemy), tk, TK_TargetEnemyNearest, TK_TargetEnemyPrev, c_habmRescue | c_habmMiner | c_habmBuilder); }
      break;

      case TK_TargetEnemyBomber:
      case TK_TargetEnemyBomberNearest:
      case TK_TargetEnemyBomberPrev: { TrekFindTarget((c_ttShip | c_ttEnemy), tk, TK_TargetEnemyBomberNearest, TK_TargetEnemyBomberPrev, c_habmThreatToStation); }
      break;

      case TK_TargetEnemyFighter:
      case TK_TargetEnemyFighterNearest:
      case TK_TargetEnemyFighterPrev: { TrekFindTarget((c_ttShip | c_ttEnemy), tk, TK_TargetEnemyFighterNearest, TK_TargetEnemyFighterPrev, c_habmFighter); }
      break;

      case TK_TargetEnemyMiner:
      case TK_TargetEnemyMinerNearest:
      case TK_TargetEnemyMinerPrev: { TrekFindTarget((c_ttShip | c_ttEnemy), tk, TK_TargetEnemyMinerNearest, TK_TargetEnemyMinerPrev, c_habmMiner); }
      break;

      case TK_TargetEnemyConstructor:
      case TK_TargetEnemyConstructorNearest:
      case TK_TargetEnemyConstructorPrev: { TrekFindTarget((c_ttShip | c_ttEnemy), tk, TK_TargetEnemyConstructorNearest, TK_TargetEnemyConstructorPrev, c_habmBuilder); }
      break;

      case TK_TargetEnemyBase:
      case TK_TargetEnemyBaseNearest:
      case TK_TargetEnemyBasePrev: { TrekFindTarget((c_ttStation | c_ttEnemy | c_ttAnyCluster), tk, TK_TargetEnemyBaseNearest, TK_TargetEnemyBasePrev, ~c_sabmPedestal); }
      break;

      case TK_TargetEnemyMajorBase:
      case TK_TargetEnemyMajorBaseNearest:
      case TK_TargetEnemyMajorBasePrev: { TrekFindTarget((c_ttStation | c_ttEnemy | c_ttAnyCluster), tk, TK_TargetEnemyMajorBaseNearest, TK_TargetEnemyMajorBasePrev, c_sabmFlag); }
      break;
      ...
      case TK_TargetFriendlyBasePrev:
      {
...

        {
          ...
          if (pht)
          {
            ...
            else if ((habm & c_habmFighter) == 0) abm = c_sabmCapLand;
          }
        }

        TrekFindTarget(ttMask, tk, TK_TargetFriendlyBaseNearest, TK_TargetFriendlyBasePrev, abm);
      }
      break;

      case TK_TargetFriendlyMajorBase:
      case TK_TargetFriendlyMajorBaseNearest:
      case TK_TargetFriendlyMajorBasePrev: { TrekFindTarget((c_ttStation | c_ttFriendly | c_ttAnyCluster), tk, TK_TargetFriendlyMajorBaseNearest, TK_TargetFriendlyMajorBasePrev, c_sabmFlag); }
      break;
      ...
      case TK_TargetPrev:
      {
        ...
        TrekFindTarget(pmodel && (pmodel != trekClient.GetShip()) ? GetSimilarTargetMask(pmodel) : (c_ttShipTypes | c_ttEnemy), tk, TK_TargetNearest, TK_TargetPrev);
      }
      break;

      case TK_TargetFriendly:
      case TK_TargetFriendlyNearest:
      case TK_TargetFriendlyPrev: { TrekFindTarget((c_ttShip | c_ttFriendly), tk, TK_TargetFriendlyNearest, TK_TargetFriendlyPrev); }
      break;

      case TK_TargetFriendlyBomber:
      case TK_TargetFriendlyBomberNearest:
      case TK_TargetFriendlyBomberPrev: { TrekFindTarget((c_ttShip | c_ttFriendly), tk, TK_TargetFriendlyBomberNearest, TK_TargetFriendlyBomberPrev, c_habmThreatToStation); }
      break;

      case TK_TargetFriendlyFighter:
      case TK_TargetFriendlyFighterNearest:
      case TK_TargetFriendlyFighterPrev: { TrekFindTarget((c_ttShip | c_ttFriendly), tk, TK_TargetFriendlyFighterNearest, TK_TargetFriendlyFighterPrev, c_habmFighter); }
      break;

      case TK_TargetFriendlyMiner:
      case TK_TargetFriendlyMinerNearest:
      case TK_TargetFriendlyMinerPrev: { TrekFindTarget((c_ttShip | c_ttFriendly), tk, TK_TargetFriendlyMinerNearest, TK_TargetFriendlyMinerPrev, c_habmMiner); }
      break;

      case TK_TargetFriendlyConstructor:
      case TK_TargetFriendlyConstructorNearest:
      case TK_TargetFriendlyConstructorPrev: { TrekFindTarget((c_ttShip | c_ttFriendly), tk, TK_TargetFriendlyConstructorNearest, TK_TargetFriendlyConstructorPrev, c_habmBuilder); }
      break;

      case TK_TargetFriendlyLifepod:
      case TK_TargetFriendlyLifepodNearest:
      case TK_TargetFriendlyLifepodPrev: { TrekFindTarget((c_ttShip | c_ttFriendly), tk, TK_TargetFriendlyLifepodNearest, TK_TargetFriendlyLifepodPrev, c_habmLifepod); }
      break;

      case TK_TargetArtifact:
      case TK_TargetArtifactNearest:
      case TK_TargetArtifactPrev: { TrekFindTarget((c_ttTreasure | c_ttNeutral), tk, TK_TargetArtifactNearest, TK_TargetArtifactPrev, 1); }
      break;

      case TK_TargetFlag:
      case TK_TargetFlagNearest:
      case TK_TargetFlagPrev: { TrekFindTarget((c_ttTreasure | c_ttEnemy), tk, TK_TargetFlagNearest, TK_TargetFlagPrev, 1); }
      break;

      case TK_TargetWarp:
      case TK_TargetWarpNearest:
      case TK_TargetWarpPrev: { TrekFindTarget((c_ttWarp | c_ttNeutral), tk, TK_TargetWarpNearest, TK_TargetWarpPrev); }
      break;
      ...
      case TK_ToggleAutoPilot:
      {
        ...
        else if ((trekClient.GetShip()->GetParentShip() == nullptr) && (trekClient.GetShip()->GetStation() == nullptr))
        {
      ...
      case TK_VoteYes: { trekClient.Vote(true); }
      break;

      case TK_VoteNo: { trekClient.Vote(false); }
      break;
      ...
      case TK_RejectCommand: { RejectQueuedCommand(true, true); }
      break;

      case TK_ClearCommand: // clear accepted command
      {
        SetAccepted(nullptr, c_cidNone);
      }
      break;

      case TK_TargetCommand: { SetTarget(trekClient.GetShip()->GetCommandTarget(c_cmdAccepted), trekClient.GetShip()->GetCommandID(c_cmdAccepted)); }
      break;

      case TK_LODUp: { m_pscrollPane->PageUp(); }
      break;

      case TK_LODDown: { m_pscrollPane->PageDown(); }
      break;

      case TK_Suicide: { suicideF = true; }
      break;

      case TK_ToggleGrid: { m_bCommandGrid = !m_bCommandGrid; }
      break;

      case TK_TargetSelf: { SetTarget(trekClient.GetShip(), c_cidNone); }
      break;

      case TK_TargetNothing: { SetTarget(nullptr, c_cidNone); }
      break;
      ...
      case TK_OccupyNextTurret:
      {
        ...
        {
          trekClient.SetMessageType(BaseClient::c_mtGuaranteed);
          BEGIN_PFM_CREATE(trekClient.m_fm, pfmBoardShip, C, BOARD_SHIP) END_PFM_CREATE;
    ...
    if (suicideF)
    {
      ...
      if (m_suicideCount++ == 0) m_suicideTime = Time::Now();
      ...
      else if (m_suicideCount == 3)
      {
        ...
        {
          trekClient.SetMessageType(BaseClient::c_mtGuaranteed);
          BEGIN_PFM_CREATE(trekClient.m_fm, pfm, C, SUICIDE) END_PFM_CREATE
        }
        else trekClient.KillShipEvent(Time::Now(), trekClient.GetShip(), nullptr, 1.0f, trekClient.GetShip()->GetPosition(), Vector::GetZero());
      }
    }
    else m_suicideCount = 0;
  }

  void RejectQueuedCommand(bool bClear = true, bool bNegative = false) override
  {
    if (trekClient.GetLastMoneyRequest() != 0)
    {
      ...
      if (pshipSender)
      {
        if (bNegative) { if (pshipSender != trekClient.GetShip()) { trekClient.SendChat(trekClient.GetShip(), CHAT_INDIVIDUAL, pshipSender->GetObjectID(), voNegativeSound, "Negative."); } }
        trekClient.SetLastSender(nullptr);
      }
    }
    else if (trekClient.GetShip()->GetCommandID(c_cmdQueued) != c_cidNone)
    {
      if (bClear) trekClient.GetShip()->SetCommand(c_cmdQueued, nullptr, c_cidNone);
  ...
  void AcceptQueuedCommand(bool bExecute) override
  {
    IshipIGC *pshipSender = trekClient.GetLastSender();
    ...
    if (request != 0)
    {
      if (pshipSender && (pshipSender != trekClient.GetShip()))
      {
        ...
        if (request > 0)
        {
          ...
          if (pplayerSender) trekClient.DonateMoney(pplayerSender, request);
        }
    ...
    else
    {
      ...
      if (cid != c_cidNone)
      {
        ...
        if (pmodel && trekClient.GetShip()->LegalCommand(cid, pmodel))
        {
          ...
          if (cid == c_cidJoin) { if (pmodel->GetObjectType() == OT_ship) trekClient.SetWing(static_cast<IshipIGC *>(pmodel)->GetWingID()); }
          else
          {
            if ((cid == c_cidPickup) && (pmodel->GetObjectType() == OT_ship) && trekClient.GetShip()->GetBaseHullType()->HasCapability(c_habmRescue))
            {
              //Tell the other person to pick me up as well
              trekClient.SendChat(trekClient.GetShip(), CHAT_INDIVIDUAL, pmodel->GetObjectID(), NA, nullptr, c_cidPickup, OT_ship, trekClient.GetShipID(), trekClient.GetShip());
            }

            SetTarget(pmodel, cid);
            SetAccepted(pmodel, cid);

            if (bExecute && (trekClient.GetShip()->GetStation() == nullptr) && (trekClient.GetShip()->GetParentShip() == nullptr) && trekClient.GetCluster(trekClient.GetShip(), pmodel))
            {
              trekClient.SetAutoPilot(true);
              trekClient.bInitTrekJoyStick = true;
              trekClient.PlaySoundEffect(salAutopilotEngageSound);
          ...
          trekClient.GetShip()->SetCommand(c_cmdQueued, nullptr, c_cidNone);
        }
      }
    }
    ...
    if (pshipSender)
    {
      if (pshipSender != trekClient.GetShip()) { trekClient.SendChat(trekClient.GetShip(), CHAT_INDIVIDUAL, pshipSender->GetObjectID(), voAffirmativeSound, "Affirmative."); }
      trekClient.SetLastSender(nullptr);
    }
  ...
  void SetQueuedCommand(IshipIGC *pshipSender, CommandID cid, ImodelIGC *pmodelTarget) override
  {
    if (pshipSender != trekClient.GetLastSender()) RejectQueuedCommand();

    if (!Training::IsTraining()) m_timeRejectQueuedCommand = trekClient.m_now + c_dtRejectQueuedCommand;
  ...
  void SetQueuedCommand(IshipIGC *pshipSender, Money money, HullID hid) override
  {
    if (pshipSender != trekClient.GetLastSender()) RejectQueuedCommand();
    ...
    if ((pshipSender != trekClient.GetShip()) && GetWindow()->GetConsoleImage())
    {
      ...
      trekClient.PostText(true, "%s", static_cast<const char *>((pshipSender->GetName() + c_str1) + (ZString(money) + c_str2) + (pht->GetName() + c_str3)));
    }
  }

  TRef<ChatListPane> m_pchatListPane;

  void SetChatListPane(ChatListPane *pchatListPane) override { m_pchatListPane = pchatListPane; }

  void SetLobbyChatTarget(ChatTarget ct) override { m_ctLobbyChat = ct; }

  ChatTarget GetLobbyChatTarget() override { return m_ctLobbyChat; }

  float MapJoystick(float value)
  {
    if (value > g_fJoystickDeadZone) value = (value - g_fJoystickDeadZone) / g_fInverseJoystickDeadZone;
    else if (value < -g_fJoystickDeadZone) value = (value + g_fJoystickDeadZone) / g_fInverseJoystickDeadZone;
    else value = 0.0f;

    if (!m_bLinearControls) value *= fabsf(value);
  ...
  {
    if (m_ptrekInput->IsAxisValid(3)) trekClient.joyThrottle = true;
    else trekClient.trekThrottle = GetThrottle(trekClient.GetShip());
  }
  ...
  bool SenseJoystick(JoystickResults *js, bool bInternalCamera, bool bReadKeyboard)
  {
    ...
    if (m_bFlipY) js->controls.jsValues[c_axisPitch] = -js->controls.jsValues[c_axisPitch];
    ...
    if (bInternalCamera)
    {
      ...
      if (m_ptrekInput->IsTrekKeyDown(TK_RollRight, bReadKeyboard)) js->controls.jsValues[c_axisRoll] = -1.0f;
      else if (m_ptrekInput->IsTrekKeyDown(TK_RollLeft, bReadKeyboard)) js->controls.jsValues[c_axisRoll] = 1.0f;

      // yaw left/right

      if (m_ptrekInput->IsTrekKeyDown(TK_RollModifier, bReadKeyboard))
      {
        if (m_ptrekInput->IsTrekKeyDown(TK_YawRight, bReadKeyboard)) js->controls.jsValues[c_axisRoll] = -1.0f;
        else if (m_ptrekInput->IsTrekKeyDown(TK_YawLeft, bReadKeyboard)) js->controls.jsValues[c_axisRoll] = 1.0f;
      }
      else
      {
        if (m_ptrekInput->IsTrekKeyDown(TK_YawRight, bReadKeyboard)) js->controls.jsValues[c_axisYaw] = -1.0f;
        else if (m_ptrekInput->IsTrekKeyDown(TK_YawLeft, bReadKeyboard)) js->controls.jsValues[c_axisYaw] = 1.0f;
      }

      // up/down == pitch

      if (m_ptrekInput->IsTrekKeyDown(TK_PitchDown, bReadKeyboard)) js->controls.jsValues[c_axisPitch] = -1.0f;
      else if (m_ptrekInput->IsTrekKeyDown(TK_PitchUp, bReadKeyboard)) js->controls.jsValues[c_axisPitch] = 1.0f;
    ...
    if (trekClient.joyThrottle) trekClient.fOldJoyThrottle = js->controls.jsValues[c_axisThrottle];
    else js->controls.jsValues[c_axisThrottle] = trekClient.trekThrottle;
  ...
  HRESULT HandleMsg(FEDMESSAGE *pfm, Time lastUpdate, Time now) override
  {
    ...
    switch (pfm->fmid)
    {
      case FM_S_LOGONACK:
      {
        ...
        if (!pfmLogonAck->fValidated) hr = S_FALSE;// don't do anything mission control will handle
      }
      break;
      ...
      case FM_S_GAME_OVER:
      {
        ...
        TriggerMusic(pfmGameOver->iSideWinner == trekClient.GetSide()->GetObjectID() ? gameOverWonMusicSound : gameOverLostMusicSound);
        trekClient.SetGameoverInfo(pfmGameOver);

        if (trekClient.GetSideID() != SIDE_TEAMLOBBY) screen(ScreenIDGameOverScreen);
      }
      break;
      ...
      case FM_S_GAME_OVER_PLAYERS:
      {
        ...
        auto vPlayerEndgameInfo = (PlayerEndgameInfo *) FM_VAR_REF(pfmGameoverPlayers, rgPlayerInfo);
        trekClient.AddGameoverPlayers(vPlayerEndgameInfo, nCount);
      }
      break;

      // ICQ integration removed - service defunct
  ...
  void OnDelPlayer(MissionInfo *pMissionInfo, SideID sideID, PlayerInfo *pPlayerInfo, QuitSideReason reason, const char *szMessageParam) override { if (pPlayerInfo->ShipID() == trekClient.GetShipID() && trekClient.GetSideID() == SIDE_TEAMLOBBY) m_nLastCountdown = c_nCountdownMax; }

  void OnMissionCountdown(MissionInfo *pMissionDef) override
  {
    m_nLastCountdown = c_nCountdownMax;
  ...
  {
    if (trekClient.MyMission() && (trekClient.MyMission()->GetStage() == STAGE_STARTING || (trekClient.MyMission()->GetStage() == STAGE_STARTED && trekClient.MyMission()->GetMissionParams().IsCountdownGame())))
    {
      // note: have the timer lag by 1 second to give users the familiar countdown feel
      int nTimeLeft;

      if (trekClient.MyMission()->GetStage() == STAGE_STARTING) nTimeLeft = max(0, int(trekClient.MyMission()->GetMissionParams().timeStart - Time::Now()) + 1);
      else { nTimeLeft = max(0, int( trekClient.MyMission()->GetMissionParams().GetCountDownTime() - (Time::Now() - trekClient.MyMission()->GetMissionParams().timeStart) + 1)); }
  ...
  void OnChatMessageChange() override { this->ChangeChatMessage(); }
  ...
  VOID VTSetText(LPSTR szFormat, ...) override
  {
    TCHAR szText[1024];
    va_list vaList;
    INT cchText;

    if (nullptr != m_hwndVTEdit)
    {
  ...
  void SoundEngineUpdate(void) override { ZSucceeded(m_pSoundEngine->Update ()); }

  TRef<ISoundInstance> StartSound(ISoundTemplate *ptemplate, ISoundPositionSource *psource = nullptr) override
  {
    if (ptemplate)
    {
      TRef<ISoundInstance> pSoundInstance;

      if (ZSucceeded(ptemplate->CreateSound(pSoundInstance, m_pSoundEngine->GetBufferSource(), psource))) return pSoundInstance;
    }
  ...
  TRef<ISoundInstance> StartSound(SoundID soundId, ISoundPositionSource *psource = nullptr) override
  {
  ...
  void StartLockDown(const ZString &strReason) override
  {
    ZAssert(m_pmessageBoxLockdown == NULL || trekClient.IsLockedDown());

    if (m_pmessageBoxLockdown) GetPopupContainer()->ClosePopup(m_pmessageBoxLockdown);

    if ((GetViewMode() != vmOverride) && (GetViewMode() != vmUI))
    {
      m_pmessageBoxLockdown = CreateMessageBox(strReason, nullptr, false, false, 1.0f);
      GetWindow()->SetWaitCursor();
      GetPopupContainer()->OpenPopup(m_pmessageBoxLockdown, false);
    }
  }

  void EndLockDown() override
  {
...
TRef<TrekWindow> TrekWindow::Create(EffectApp *papp, const ZString &strCommandLine, bool bMovies, bool bSoftware, bool bHardware, bool bPrimary, bool bSecondary) { return new TrekWindowImpl(papp, strCommandLine, bMovies, bSoftware, bHardware, bPrimary, bSecondary); }

// forward squad message to squad screen
void ForwardSquadMessage(FEDMESSAGE *pSquadMessage);

void WinTrekClient::ForwardSquadMessage(FEDMESSAGE *pSquadMessage) { ::ForwardSquadMessage(pSquadMessage); }

// forward CharacterInfo message to CharacterInfo screen
void ForwardCharInfoMessage(FEDMESSAGE *pCharInfoMessage);

void WinTrekClient::ForwardCharInfoMessage(FEDMESSAGE *pCharInfoMessage) { ::ForwardCharInfoMessage(pCharInfoMessage); }

// forward LeaderBoard message to the LeaderBoard screen
void ForwardLeaderBoardMessage(FEDMESSAGE *pCharInfoMessage);

void WinTrekClient::ForwardLeaderBoardMessage(FEDMESSAGE *pLeaderBoardMessage) { ::ForwardLeaderBoardMessage(pLeaderBoardMessage); }