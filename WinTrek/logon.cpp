...
class LogonPopup : public IPopup, public EventTargetContainer<LogonPopup>, public TEvent<ZString>::Sink
{
  TRef<Pane> m_ppane;
...
public:
  LogonPopup(Modeler *pmodeler, LogonSite *psite, LogonType lt, LPCSTR szPrompt, LPCSTR szName, LPCSTR szPW, BOOL fRememberPW)
    : m_psite(psite),
      m_lt(lt)
  {
    enum LogonNameType
    ...
    if (lt == LogonLAN) logonNameType = lntCallsign;
    else if (trekClient.GetCfgInfo().bUsePassport) logonNameType = lntPassport;
    else logonNameType = lntZone;
    ...
    pnsLogonData->AddMember("Callsign", new ModifiableNumber(static_cast<float>(lntCallsign)));
    pnsLogonData->AddMember("ZoneID", new ModifiableNumber(static_cast<float>(lntZone)));
    pnsLogonData->AddMember("PassportID", new ModifiableNumber(static_cast<float>(lntPassport)));
    pnsLogonData->AddMember("logonNameType", new ModifiableNumber(static_cast<float>(logonNameType)));
    ...
    CastTo(m_peditName, static_cast<Pane *>(pns->FindMember("nameEditPane")));
    ...
    CastTo(m_peditPassword, static_cast<Pane *>(pns->FindMember("passwordEditPane")));
    ...
    m_peditName->SetMaxLength((LogonLAN != lt && trekClient.GetCfgInfo().bUsePassport) ? (c_cbPassportName - 1) : (c_cbName - 1));

    if (m_peditName->GetString().IsEmpty()) { SetFocus(m_peditName); }
    else { SetFocus(m_peditPassword); }
  ...
  {
    if (m_pfocus) { m_pfocus->SetFocusState(false); }
    m_pfocus = pfocus;
    m_pfocus->SetFocusState(true);
  }

  bool OnEvent(TEvent<ZString>::Source *psource, ZString str) override
  {
  ...
  void SetContainer(IPopupContainer *pcontainer) override
  {
  ...
  bool OnKey(IInputProvider *pprovider, const KeyState &ks, bool &fForceTranslate) override
  {
    if (ks.bDown)
    {
      if (ks.vk == VK_RETURN)
      {
        if (m_pbuttonLogon->GetEnabled()) { OnButtonLogon(); }
        else { OnButtonAbort(); }
        return true;
      }
      if (ks.vk == VK_ESCAPE)
      ...
      if (ks.vk == VK_TAB)
      {
        if (m_pfocus == m_peditName && (LogonAllegianceZone == m_lt || LogonFreeZone == m_lt)) { SetFocus(m_peditPassword); }
        else { SetFocus(m_peditName); }
        return true;
      }
    }
  ...
  bool OnChar(IInputProvider *pprovider, const KeyState &ks) override { return m_pfocus->OnChar(pprovider, ks); }
};
...
TRef<IPopup> CreateLogonPopup(Modeler *pmodeler, LogonSite *psite, LogonType lt, LPCSTR szPrompt, LPCSTR szName, LPCSTR szPW, BOOL fRememberPW) { return new LogonPopup(pmodeler, psite, lt, szPrompt, szName, szPW, fRememberPW); }