...
class TrainingScreen : public Screen, public EventTargetContainer<TrainingScreen>, public TrekClientEventSink, public IItemEvent::Sink
{
  TRef<Pane> m_ppane;
...
public:
  ...
  class TrainLiveDialogPopup : public IPopup, public EventTargetContainer<TrainLiveDialogPopup>, public TrekClientEventSink
  {
    TRef<Pane> m_ppane;
  ...
  public:
    TrainLiveDialogPopup(TRef<INameSpace> pns, TrainingScreen *pparent)
    {
      ...
      AddEventTarget(&TrainLiveDialogPopup::OnButtonOk, m_pbuttonOk->GetEventSource());
      AddEventTarget(&TrainLiveDialogPopup::OnButtonCancel, m_pbuttonCancel->GetEventSource());
      AddEventTarget(&TrainLiveDialogPopup::OnButtonCancel, m_pbuttonCancel2->GetEventSource());
    ...
    Pane *GetPane() { return m_ppane; }

    void CloseThePopup(void)
    {
      if (m_ppopupOwner) { m_ppopupOwner->ClosePopup(this); }
      else if (m_pcontainer) { m_pcontainer->ClosePopup(this); }
  ...
  TrainingScreen(Modeler *pmodeler)
  {
    ...
    switch (m_iMissionNext)
    {
      ...
      case Training::c_TM_6_Practice_Arena:
        ...
        break;
      case Training::c_TM_7_Live: default:
  ...
  bool OnEvent(IItemEvent::Source *pevent, ItemID pitem) { return true; }
  ...
  bool OnButtonTrain()
  {
    ...
    {
      m_iMissionNext++;
      if (m_iMissionNext > Training::c_TM_6_Practice_Arena) m_iMissionNext = Training::c_TM_6_Practice_Arena;
    }
  ...
  Pane *GetPane() override { return m_ppane; }
...
#define TRAINING_MISSION_7_GAME_NAME    "Training Mission 7"
  ...
  bool DoTrainLive(void)
  {
    ...
    TRef<IMessageBox> pMsgBox = CreateMessageBox("Creating game...", nullptr, false);
    GetWindow()->GetPopupContainer()->OpenPopup(pMsgBox, false);
  ...
  bool OnTrainLive(void)
  {
    ...
    while (!GetWindow()->GetPopupContainer()->IsEmpty()) GetWindow()->GetPopupContainer()->ClosePopup(NULL);
    GetWindow()->RestoreCursor();
  ...
  void CheckForExistingGames(void)
  {
    ...
    m_spAdminGamesPtr->get_Item(&static_cast<_variant_t>(0L), &m_spAdminGamePtr);
  ...
  */
  ...
  HRESULT CreateTrainingMissionGame(void)
  {
    ...
    if (!Error(hr = spParams.CreateInstance(CLSID_AGCGameParameters)))
    {
      ...
      {
        // get the game
        if (!Error(hr = m_spAdminGamesPtr->get_Item(&static_cast<_variant_t>(0L), &m_spAdminGamePtr)))
        {
          // set up the teams
          ...
          GetWindow()->SetWaitCursor();
          TRef<IMessageBox> pMsgBox = CreateMessageBox("Connecting...", nullptr, false);
          GetWindow()->GetPopupContainer()->OpenPopup(pMsgBox, false);
  ...
  HRESULT CreateStandaloneGame(void)
  {
    ...
    if (!Error(hr = CoGetClassObject(__uuidof(AdminSession), CLSCTX_LOCAL_SERVER, nullptr, __uuidof(spClass), (void **) &spClass)))
    {
  ...
  //////////////////////////////////////////////////////////////////////////////
  static HRESULT KillStandaloneGame(void)
  {
    HRESULT hr = S_OK;
    m_spAdminGamesPtr = 0;
    if (m_spAdminGamePtr)
    {
      if (CoIsHandlerConnected(m_spAdminGamePtr)) hr = m_spAdminGamePtr->Kill();
      m_spAdminGamePtr = 0;
    }
    return hr;
  }
  ...
  void OnLogonGameServer()
  {
    ...
    GetWindow()->SetWaitCursor();
    TRef<IMessageBox> pmsgBox = CreateMessageBox("Joining mission....", nullptr, false, false);
    GetWindow()->GetPopupContainer()->OpenPopup(pmsgBox, false);
  }
...
TRef<Screen> CreateTrainingScreen(Modeler *pmodeler) { return new TrainingScreen(pmodeler); }

// destructor
void KillTrainingStandaloneGame(void) { TrainingScreen::KillStandaloneGame(); }